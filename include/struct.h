/************************************************************************
 *   Zen Internet Relay Chat Daemon, include/struct.h
...
/** These are the generic list functions that are used all around in ZenIRCd.
...
extern void zen_add_names(NameList **n, ConfigEntry *ce);
...
/** Command is for control channel only (zenircd.ctl socket) */
 ...
 * It allows ZenIRCd devs to change the parameters in the function without
 ...
 * command arguments may change in future ZenIRCd versions.
 */
 ...
 */
#define ZENDB_CRYPT_FILE_CHUNK_SIZE 4096

/** The salt length. Don't change. */
#define ZENDB_SALT_LEN 16

/** Database modes of operation (read or write)
 * @ingroup ZenDBFunctions
 */
typedef enum ZenDBMode {
	ZENDB_MODE_READ = 0,
	ZENDB_MODE_WRITE = 1
} ZenDBMode;

typedef enum ZenDBCipher {
	ZENDB_CIPHER_XCHACHA20 = 0x0001
} ZenDBCipher;

typedef enum ZenDBKDF {
	ZENDB_KDF_ARGON2ID = 0x0001
} ZenDBKDF;

/** Database configuration for a particular file */
typedef struct ZenDBConfig {
	...
	uint16_t saltlen; /**< Length of the salt (normally ZENDB_SALT_LEN) */
	char *salt;       /**< Salt */
...
/** Error codes returned by @ref ZenDBFunctions
 * @ingroup ZenDBFunctions
 */
typedef enum ZenDBError {
	ZENDB_ERROR_SUCCESS = 0,      /**< Success, not an error */
	ZENDB_ERROR_FILENOTFOUND = 1, /**< File does not exist */
	ZENDB_ERROR_CRYPTED = 2,      /**< File is crypted but no password provided */
	ZENDB_ERROR_NOTCRYPTED = 3,   /**< File is not crypted and a password was provided */
	ZENDB_ERROR_HEADER = 4,       /**< Header is corrupt, invalid or unknown format */
	ZENDB_ERROR_SECRET = 5,       /**< Invalid secret { } block provided - either does not exist or does not meet requirements */
	ZENDB_ERROR_PASSWORD = 6,     /**< Invalid password provided */
	ZENDB_ERROR_IO = 7,           /**< I/O error */
	ZENDB_ERROR_API = 8,          /**< API call violation, eg requesting to write on a file opened for reading */
	ZENDB_ERROR_INTERNAL = 9,     /**< Internal error, eg crypto routine returned something unexpected */
} ZenDBError;

/** Database handle
 * This is returned by zendb_open() and used by all other @ref ZenDBFunctions
 * @ingroup ZenDBFunctions
 */
...
struct Spamfilter {
	...
	char *tkl_reason;      /**< Reason to use for bans placed by this spamfilter, escaped by zen_encodespace(). */
...
struct Server {
	...
	struct {
		...
		char *software;     /**< Name of the software (eg: zenircd-X.Y.Z) */
...
extern MODVAR char *version, *infotext[], *dalinfotext[], *zencredits[], *zeninfo[];
 ...
 * the -320xx range are ZenIRCd JSON-RPC server specific error codes,
 * and -1000 and below are ZenIRCd specific application error codes.
 */
