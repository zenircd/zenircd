/************************************************************************
 *   ZenIRCd - Zen Internet Relay Chat Daemon - src/proc_io_server.c
...
/** Create the zenircd.ctl socket (server-side) */
...
CMD_FUNC(procio_rehash)
{
	...
	{
		SetMonitorRehash(client);
		zen_log(ULOG_INFO, "config", "CONFIG_RELOAD", client, "Rehashing server configuration file [./zenircd rehash]");
