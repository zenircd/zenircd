 ...
 * (C) Copyright 2019 Syzop, Gottem and the ZenIRCd team
...
int write_channel_entry(ZenDB *db, const char *tmpfname, Channel *channel);
...
int write_listmode(ZenDB *db, const char *tmpfname, Ban *lst)
{
	Ban *l;
	int cnt = 0;
	...
	for (l = lst; l; l = l->next)
		cnt++;
	W_SAFE(zendb_write_int32(db, cnt));

	for (l = lst; l; l = l->next)
	{
		/* The entry, setby, seton */
		W_SAFE(zendb_write_str(db, l->banstr));
		W_SAFE(zendb_write_str(db, l->who));
		W_SAFE(zendb_write_int64(db, l->when));
	}
	return 1;
}
...
int write_channel_entry(ZenDB *db, const char *tmpfname, Channel *channel)
{
	...
	W_SAFE(zendb_write_int32(db, MAGIC_CHANNEL_START));
	/* Channel name */
	W_SAFE(zendb_write_str(db, channel->name));
	/* Channel creation time */
	W_SAFE(zendb_write_int64(db, channel->creationtime));
	/* Topic (topic, setby, seton) */
	W_SAFE(zendb_write_str(db, channel->topic));
	W_SAFE(zendb_write_str(db, channel->topic_nick));
	W_SAFE(zendb_write_int64(db, channel->topic_time));
	/* Basic channel modes (eg: +sntkl key 55) */
	channel_modes(&me, modebuf, parabuf, sizeof(modebuf), sizeof(parabuf), channel, 1);
	W_SAFE(zendb_write_str(db, modebuf));
	W_SAFE(zendb_write_str(db, parabuf));
	/* Mode lock */
	W_SAFE(zendb_write_str(db, channel->mode_lock));
	...
	if (!write_listmode(db, tmpfname, channel->invexlist))
		return 0;
	W_SAFE(zendb_write_int32(db, MAGIC_CHANNEL_END));
	return 1;
}
...
#define R_SAFE(x) \
	...
	{ \
		if (!(x)) \
		{ \
			config_warn("[channeldb] Read error from database file '%s' (possible corruption): %s", cfg.database, zendb_get_error_string()); \
			if (e) \
			{ \
				safe_free(e->banstr); \
				safe_free(e->who); \
...
int read_listmode(ZenDB *db, Channel *channel, ExtbanType ban_type, Ban **lst)
{
	uint32_t total;
	uint64_t when;
	int i;
	...
	R_SAFE(zendb_read_int32(db, &total));

	for (i = 0; i < total; i++)
	{
		const char *str;
		e = safe_alloc(sizeof(Ban));
		R_SAFE(zendb_read_str(db, &e->banstr));
		R_SAFE(zendb_read_str(db, &e->who));
		R_SAFE(zendb_read_int64(db, &when));
...
int read_channeldb(void)
{
	...
	if (!db)
	{
		...
		} else if (zendb_get_error_code() == ZENDB_ERROR_NOTCRYPTED)
		{
			...
			if (!db)
			{
				...
				config_warn("[channeldb] Unable to open the database file '%s': %s", cfg.database, zendb_get_error_string());
				return 0;
			}
		} else
		{
			config_warn("[channeldb] Unable to open the database file '%s' for reading: %s", cfg.database, zendb_get_error_string());
			return 0;
		}
	}
	...
	R_SAFE(zendb_read_int64(db, &count));
	...
	for (i = 1; i <= count; i++)
	{
		...
		R_SAFE(zendb_read_str(db, &chname));
		R_SAFE(zendb_read_int64(db, &creationtime));
		R_SAFE(zendb_read_str(db, &topic));
		R_SAFE(zendb_read_str(db, &topic_nick));
		R_SAFE(zendb_read_int64(db, &topic_time));
		R_SAFE(zendb_read_str(db, &modes1));
		R_SAFE(zendb_read_str(db, &modes2));
		R_SAFE(zendb_read_str(db, &mode_lock));
