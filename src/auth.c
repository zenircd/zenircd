/*
 *   Zen Internet Relay Chat Daemon, src/auth.c
 *   (C) 2001 Carsten V. Munk (stskeeps@tspre.org)
 *   (C) 2003-2019 Bram Matthys (syzop@vulnscan.org) and the ZenIRCd team
 *
 ...
 */
 ...
 #pragma GCC diagnostic pop
	...
	/* Now handle ZenIRCd-style password hashes.. */
 ...
 * @note
 * - The return value was different in versions before ZenIRCd 5.0.0!
...
#define ZENIRCD_ARGON2_DEFAULT_TIME_COST        2
#define ZENIRCD_ARGON2_DEFAULT_MEMORY_COST      6144
#define ZENIRCD_ARGON2_DEFAULT_PARALLELISM_COST 2
#define ZENIRCD_ARGON2_DEFAULT_HASH_LENGTH      32
#define ZENIRCD_ARGON2_DEFAULT_SALT_LENGTH      (128 / 8)

static char *mkpass_argon2(const char *para)
{
	static char buf[512];
	char salt[ZENIRCD_ARGON2_DEFAULT_SALT_LENGTH];
	int ret, i;
	...
	*buf = '\0';
	...
	ret = argon2id_hash_encoded(ZENIRCD_ARGON2_DEFAULT_TIME_COST,
	                            ZENIRCD_ARGON2_DEFAULT_MEMORY_COST,
	                            ZENIRCD_ARGON2_DEFAULT_PARALLELISM_COST,
	                            para,
	                            strlen(para),
	                            salt,
	                            sizeof(salt),
	                            ZENIRCD_ARGON2_DEFAULT_HASH_LENGTH,
	                            buf,
	                            sizeof(buf));
...
void check_argon2(void)
{
	...
	if (WIFSIGNALED(status))
	{
		fprintf(stderr,
		        ...
		        "Usually this happens when an ZenIRCd installation is copied from\n"
		...
		{
			fprintf(stderr,
			        "To fix this, compile and install ZenIRCd on THIS machine:\n"
		...
		{
			fprintf(stderr,
			        "To fix this you need to compile and install ZenIRCd on THIS machine.\n"
