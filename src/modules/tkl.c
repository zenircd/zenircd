/*
 * Zen Internet Relay Chat Daemon, src/modules/tkl.c
 * TKL Commands: server bans, spamfilters, etc.
 * (C) 1999-2019 Bram Matthys and The ZenIRCd Team
 *
...
int _zen_match_iplist(Client *client, NameList *l);
...
MOD_TEST()
{
 ...
 #pragma GCC diagnostic pop
	...
	EfunctionAdd(modinfo->handle, EFUNC_ZEN_MATCH_IPLIST, _zen_match_iplist);
...
int tkl_config_test_spamfilter(ConfigFile *cf, ConfigEntry *ce, int type, int *errs)
{
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
		...
		} else if (!strcmp(cep->name, "match-type"))
		{
			...
			match_type = zen_match_method_strtoval(cep->value);
			if (match_type == 0)
			{
	...
	if (match && match_type)
	{
		...
		m = zen_create_match(match_type, match, &err);
		if (!m)
		{
...
int tkl_config_run_spamfilter(ConfigFile *cf, ConfigEntry *ce, int type)
{
	...
	banreason = zen_encodespace(banreason);
...
void recompile_spamfilters(void)
{
	...
	for (tkl = tklines[index]; tkl; tkl = tkl->next)
	{
		if (!TKLIsSpamfilter(tkl) || (tkl->ptr.spamfilter->match->type != MATCH_PCRE_REGEX))
			continue;
		m = zen_create_match(MATCH_PCRE_REGEX, tkl->ptr.spamfilter->match->str, &err);
		if (!m)
		{
		...
		zen_delete_match(tkl->ptr.spamfilter->match); /* unset old one */
...
static const char *spamfilter_fallback_id(TKL *tkl)
{
	...
	static const char key[16] = "ZenIRCd rocks";
	static char buf[16];
	const char *prefix;
	int prefixlen;
	char content[8192];
	...
	snprintf(content, sizeof(content), "%s\t%s\t%s\t%s",
	         ...
	         actbuf,
	         zen_match_method_valtostr(tkl->ptr.spamfilter->match->type),
...
void spamfilter_del_by_id(Client *client, const char *id)
{
	...
	tkllayer[8] = "-";
	tkllayer[9] = "-";
	tkllayer[10] = zen_match_method_valtostr(tk->ptr.spamfilter->match->type); /* matching type */
...
CMD_FUNC(cmd_spamfilter)
{
	...
	match_type = zen_match_method_strtoval(parv[2] + 1);
	if (!match_type)
	{
		spamfilter_new_usage(client, parv);
		return;
	...
	if (add)
	{
		...
		m = zen_create_match(match_type, parv[7], &err);
		if (!m)
		{
	...
	if (parv[6][0] == '-')
		strlcpy(reason, zen_encodespace(SPAMFILTER_BAN_REASON), sizeof(reason));
	else
	...
	if ((n > 500) && add)
	{
		sendnotice(client, "Sorry, spamfilter too long. You'll either have to trim down the "
		                   "reason or the regex (exceeded by %d bytes)",
		           n - 500);
		return;
	}
...
void _free_tkl(TKL *tkl)
{
	...
	} else if (TKLIsSpamfilter(tkl) && tkl->ptr.spamfilter)
	{
		...
		if (tkl->ptr.spamfilter->match)
			zen_delete_match(tkl->ptr.spamfilter->match);
...
char *_tkl_uhost(TKL *tkl, char *buf, size_t buflen, int options)
{
	if (TKLIsServerBan(tkl))
	{
		...
		} else if (is_extended_server_ban(tkl->ptr.serverban->usermask))
		{
	...
	} else if (TKLIsBanException(tkl))
	{
		...
		{
			ircsnprintf(buf, buflen, "<match-item>");
		} else if (is_extended_server_ban(tkl->ptr.banexception->usermask))
		{
			ircsnprintf(buf, buflen, "%s%s%s",
 ...
 * @note Return value changed with regards to ZenIRCd 4!
 */
int _find_tkline_match(Client *client, int skip_soft)
{
	TKL *tkl;
...
int spamfilter_check_users(TKL *tkl)
{
	...
	list_for_each_entry_reverse(client, &lclient_list, lclient_node)
	{
		if (MyUser(client))
		{
			...
			spamfilter_build_user_string(spamfilter_user, client->name, client);
			if (!zen_match(tkl->ptr.spamfilter->match, spamfilter_user, &regex_error))
			{
				if (regex_error)
 ...
 * Note: function prototype changed as per ZenIRCd 4.2.0.
...
CMD_FUNC(cmd_tkl_add)
{
	...
	} else if (TKLIsSpamfilterType(type))
	{
		...
		match_method = zen_match_method_strtoval(parv[10]);
		if (match_method == 0)
		{
			zen_log(ULOG_WARNING, "tkl", "TKL_ADD_INVALID", client,
			        "Invalid TKL entry from $client: "
		...
		{
			m = zen_create_match(match_method, match_string, &err);
			if (!m)
			{
				zen_log(ULOG_WARNING, "tkl", "TKL_ADD_INVALID", client,
				        "Invalid TKL entry from $client: "
...
CMD_FUNC(cmd_tkl_del)
{
	...
	} else
	{
		 ...
		 * to ZenIRCd but forgot to add the removal code :D.
		 */
		abort();
	}
 ...
 *
 * [A] tkl reason field must be escaped by caller [eg: use zen_encodespace()
...
int _join_viruschan(Client *client, TKL *tkl, int type)
{
	...
	sendnotice(client, "You are now restricted to talking in %s: %s",
	           SPAMFILTER_VIRUSCHAN, zen_decodespace(tkl->ptr.spamfilter->tkl_reason));
	...
	if (channel)
	{
		...
		ircsnprintf(buf, sizeof(buf), "[Spamfilter] %s matched filter '%s' [%s] [%s]",
		            ...
		            zen_decodespace(tkl->ptr.spamfilter->tkl_reason));
...
static int spamfilter_hide_content(int target, SpamfilterShowMessageContentOnHit setting)
{
	if ((target == SPAMF_USERMSG) || (target == SPAMF_USERNOTICE))
	{
		if (setting == SPAMFILTER_SHOW_MESSAGE_CONTENT_ON_HIT_ALWAYS)
			return 0;
		return 1;
	} else if ((target == SPAMF_CHANMSG) || (target == SPAMF_CHANNOTICE))
	{
		if ((setting == SPAMFILTER_SHOW_MESSAGE_CONTENT_ON_HIT_ALWAYS) ||
...
void _run_deferred_rule_only_spamfilters(Client *client)
{
	...
	{
		char *reason = zen_decodespace(winner_tkl->ptr.spamfilter->tkl_reason);
...
int _match_spamfilter(Client *client, const char *str_in, int target, const char *cmd, const char *destination, int flags, ClientContext *clictx, TKL **rettkl)
{
	...
	for (tkl = tklines[tkl_hash('F')]; tkl; tkl = tkl->next)
	{
		...
		if (tkl->ptr.spamfilter->match && (tkl->ptr.spamfilter->match->type != MATCH_NONE))
		{
			...
			if (tkl->ptr.spamfilter->input_conversion == INPUT_CONVERSION_STRIP_CONTROL_CODES)
				ret = zen_match(tkl->ptr.spamfilter->match, str, &regex_error); /* StripControlCodes() */
			else if (tkl->ptr.spamfilter->input_conversion == INPUT_CONVERSION_CONFUSABLES)
				ret = zen_match(tkl->ptr.spamfilter->match, str_deconfused ? str_deconfused : str, &regex_error); /* utf8_convert_confusables(), with fallback */
			else
				ret = zen_match(tkl->ptr.spamfilter->match, str_in, &regex_error); /* raw */
	...
	/* Spamfilter matched */
	reason = zen_decodespace(tkl->ptr.spamfilter->tkl_reason);
	...
	if (!IsDead(client))
	{
		if ((ret == BAN_ACT_BLOCK) || (ret == BAN_ACT_SOFT_BLOCK))
		{
			...
			return ret;
		} else if ((ret == BAN_ACT_WARN) || (ret == BAN_ACT_SOFT_WARN))
		{
			if (content_revealed &&
			...
			return 0;
		} else if ((ret == BAN_ACT_DCCBLOCK) || (ret == BAN_ACT_SOFT_DCCBLOCK))
		{
			if (target == SPAMF_DCC)
			{
			...
			return ret;
		} else if ((ret == BAN_ACT_VIRUSCHAN) || (ret == BAN_ACT_SOFT_VIRUSCHAN))
		{
...
int _zen_match_iplist(Client *client, NameList *l)
{
	...
	for (; l; l = l->next)
	{
		...
		} else if (strchr(mask, ':'))
		{
			...
			} else if (comp_with_mask(clientip, maskip, cidr))
			{
