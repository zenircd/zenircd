...
CMD_FUNC(cmd_slog);
void _do_zen_log_remote_deliver(LogLevel loglevel, const char *subsystem, const char *event_id, MultiLine *msg, const char *json_serialized);

MOD_TEST()
{
	MARK_AS_OFFICIAL_MODULE(modinfo);
	EfunctionAddVoid(modinfo->handle, EFUNC_DO_ZEN_LOG_REMOTE_DELIVER, _do_zen_log_remote_deliver);
	return MOD_SUCCESS;
}
 ...
 * and to contain a valid ZenIRCd JSON log.
...
CMD_FUNC(cmd_slog)
{
	...
	if (!json_incoming)
		return;
	// Was previously: zen_log_raw(loglevel, subsystem, event_id, NULL, msg); // WRONG: this may re-broadcast too, so twice, including back to direction!!!
	...
	if (json_serialized)
		do_zen_log_internal_from_remote(loglevel, subsystem, event_id, mmsg, j, json_serialized, client);
