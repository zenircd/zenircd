/*
 *   Zen Internet Relay Chat Daemon, src/support.c
 ...
 *      zen_log(ULOG_INFO, "test", "TEST", "Got: $name", log_data_string(name));
 * @endcode
 */
...
/* Natural sort case comparison routines. The following copyright header applies:
 ...
 The code was modified / ZenIRCderized by Bram Matthys ("Syzop").
 ...
 */
char *zen_mktemp(const char *dir, const char *suffix)
{
	FILE *fd;
	unsigned int i;
 ...
 */
char *zen_getpathname(const char *filepath, char *path)
{
 ...
 */
const char *zen_getfilename(const char *path)
{
	int len = strlen(path);
	const char *end;
 ...
 */
int zen_mkdir(const char *pathname, mode_t mode)
{
#ifdef _WIN32
 ...
 * @param dname	The directory name, eg /home/irc/zenircd/logs/2022/08/05
 * @param mode	The mode to create with, eg 0777. Ignored on Windows.
 * @returns 1 on success, 0 on failure.
 */
int zen_create_directory_structure(const char *dname, mode_t mode)
{
	if (zen_mkdir(dname, mode) == 0)
	{
 ...
 * @param fname	The full path name, eg /home/irc/zenircd/logs/2022/08/05/ircd.log
 * @param mode	The mode to create with, eg 0777. Ignored on Windows.
 * @notes This is used as an easier way to call zen_create_directory_structure()
 *        if you have a filename instead of the directory part.
 * @returns 1 on success, 0 on failure.
 */
int zen_create_directory_structure_for_file(const char *fname, mode_t mode)
{
	char buf[PATH_MAX + 1];
	const char *path = zen_getpathname(fname, buf);
	if (!path)
		return 0;
	return zen_create_directory_structure(path, mode);
}

/** Returns the special module tmp name for a given path.
 * The original string is not modified.
 */
const char *zen_getmodfilename(const char *path)
{
	static char ret[512];
	char buf[512];
	char *p;
 ...
 */
const char *zen_mkcache(const char *url)
{
	static char tempbuf[PATH_MAX + 1];
	char tmp2[128];
...
{
	return file_exists(zen_mkcache(url));
}
 ...
 */
int zen_copyfile(const char *src, const char *dest)
{
	char buf[16384];
	time_t mtime;
	int srcfd, destfd, len;

	mtime = zen_getfilemodtime(src);
	...
	close(srcfd);
	close(destfd);
	zen_setfilemodtime(dest, mtime);
	return 1;
}

/** Same as zen_copyfile, but with an option to try hardlinking first */
int zen_copyfileex(const char *src, const char *dest, int tryhardlink)
{
	unlink(dest);
#ifndef _WIN32
	...
	if (tryhardlink && !link(src, dest))
		return 1; /* success */
#endif
	return zen_copyfile(src, dest);
}

/** Set the modification time on a file */
void zen_setfilemodtime(const char *filename, time_t mtime)
{
#ifndef _WIN32
	struct utimbuf utb;
...
time_t zen_getfilemodtime(const char *filename)
{
#ifndef _WIN32
	struct stat sb;
	if (stat(filename, &sb))
...
int zen_touch(const char *filename, time_t mtime)
{
	FILE *fd = fopen(filename, "a");
	if (!fd)
		return 0;
	fclose(fd);
	zen_setfilemodtime(filename, mtime);
	return 1;
}
...
char *zen_strftime(const char *str)
{
	time_t t;
	struct tm *tmp;
	static char buf[512];
