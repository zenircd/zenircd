/*
 *   Zen Internet Relay Chat Daemon, src/modules/vhost.c
 *   (C) 2000-2024 Carsten V. Munk, Bram Matthys and the ZenIRCd Team
 *
...
void do_vhost(Client *client, ConfigItem_vhost *vhost)
{
	...
	*newhost = '\0';
	zen_expand_string(vhost->virthost, newhost, sizeof(newhost), NULL, 0, client);
	 ...
	 *  in the zen_log() call currently below).
	 */
	if (vhost->login)
	{
		if (vhost->virtuser)
