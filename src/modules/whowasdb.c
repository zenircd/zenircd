...
#define W_SAFE_PROPERTY(db, x, y) \
	do \
	{ \
		if (x && y && (!zendb_write_str(db, x) || !zendb_write_str(db, y))) \
		{ \
			WARN_WRITE_ERROR(tmpfname); \
			zendb_close(db); \
			return 0; \
...
int write_whowas_entry(ZenDB *db, const char *tmpfname, WhoWas *e);
...
int write_whowasdb(void)
{
	...
	W_SAFE(zendb_write_int32(db, WHOWASDB_HEADER));
	W_SAFE(zendb_write_int32(db, whowasdb_version));
...
int write_whowas_entry(ZenDB *db, const char *tmpfname, WhoWas *e)
{
	...
	W_SAFE(zendb_write_int32(db, MAGIC_WHOWASDB_START));
	...
	W_SAFE(zendb_write_int32(db, MAGIC_WHOWASDB_END));
	return 1;
}
...
#define R_SAFE(x) \
	...
	{ \
		if (!(x)) \
		{ \
			config_warn("[whowasdb] Read error from database file '%s' (possible corruption): %s", cfg.database, zendb_get_error_string()); \
			zendb_close(db); \
			FreeWhowasEntry(); \
			return 0; \
		} \
...
int read_whowasdb(void)
{
	...
	if (!db)
	{
		...
		} else if (zendb_get_error_code() == ZENDB_ERROR_NOTCRYPTED)
		{
			...
			if (!db)
			{
				...
				config_warn("[whowasdb] Unable to open the database file '%s': %s", cfg.database, zendb_get_error_string());
				return 0;
			}
		} else
		{
			config_warn("[whowasdb] Unable to open the database file '%s' for reading: %s", cfg.database, zendb_get_error_string());
			return 0;
		}
	}
	...
	for (i = 1; i <= count; i++)
	{
		...
		while (1)
		{
			R_SAFE(zendb_read_str(db, &key));
			R_SAFE(zendb_read_str(db, &value));
			if (!strcmp(key, "nick"))
			{
		...
		if (nick && username && hostname && realname)
		{
			...
			if (e->hashv != -1)
				free_whowas_entry(e);
			/* Set values */
			//zen_log(ULOG_DEBUG, "whowasdb", "WHOWASDB_READ_RECORD", NULL,
