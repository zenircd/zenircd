 ...
 * (C) 2021-.. Bram Matthys (Syzop) and the ZenIRCd Team
 ...
 * In ZenIRCd, this must be used instead of json_string()
 ...
 */
json_t *json_string_zen(const char *s)
{
...
void json_expand_client_security_groups(json_t *parent, Client *client)
{
	...
	if (user_allowed_by_security_group_name(client, "known-users"))
		json_array_append_new(child, json_string_zen("known-users"));
	else
		json_array_append_new(child, json_string_zen("unknown-users"));

	for (s = securitygroups; s; s = s->next)
		if (strcmp(s->name, "known-users") && user_allowed_by_security_group(client, s))
			json_array_append_new(child, json_string_zen(s->name));
}
...
void json_expand_client(json_t *j, const char *key, Client *client, int detail)
{
	...
	json_object_set_new(child, "name", json_string_zen(client->name));
	json_object_set_new(child, "id", json_string_zen(client->id));
	...
	if (client->user && *client->user->realhost)
		json_object_set_new(child, "hostname", json_string_zen(client->user->realhost));
	else if (client->local && *client->local->sockhost)
		json_object_set_new(child, "hostname", json_string_zen(client->local->sockhost));
	else
		json_object_set_new(child, "hostname", json_string_zen(GetIP(client)));

	/* same for ip, is there for all (well, some services pseudo-users may not have one) */
	json_object_set_new(child, "ip", json_string_zen(client->ip));
	...
	if (client->user)
	{
		...
		json_object_set_new(child, "details", json_string_zen(buf));
	} else if (client->ip)
	{
		if (*client->name)
	...
	{
		json_object_set_new(child, "details", json_string_zen(client->name));
	}
	...
	if (client->user)
	{
		...
		json_object_set_new(user, "username", json_string_zen(client->user->username));
		if (!BadPtr(client->info))
			json_object_set_new(user, "realname", json_string_zen(client->info));
		if (has_user_mode(client, 'x') && client->user->virthost && strcmp(client->user->virthost, client->user->realhost))
			json_object_set_new(user, "vhost", json_string_zen(client->user->virthost));
		if (*client->user->cloakedhost)
			json_object_set_new(user, "cloakedhost", json_string_zen(client->user->cloakedhost));
		if (client->uplink)
			json_object_set_new(user, "servername", json_string_zen(client->uplink->name));
		if (IsLoggedIn(client))
			json_object_set_new(user, "account", json_string_zen(client->user->account));
		json_object_set_new(user, "reputation", json_integer(GetReputation(client)));
		json_expand_client_security_groups(user, client);
		if (client->user->away)
		{
			json_object_set_new(user, "away_reason", json_string_zen(client->user->away));
		...
		json_object_set_new(user, "modes", json_string_zen(buf + 1));
		if (client->user->snomask)
			json_object_set_new(user, "snomasks", json_string_zen(client->user->snomask));

		/* if oper then we can possibly expand a bit more */
		str = get_operlogin(client);
		if (str)
			json_object_set_new(user, "operlogin", json_string_zen(str));
		str = get_operclass(client);
		if (str)
			json_object_set_new(user, "operclass", json_string_zen(str));
		...
		if (detail > 2)
		{
			...
			if (detail == 3)
			{
				...
				for (m = client->user->channel; m; m = m->next)
				{
					...
					{
						/* Truncated */
						json_array_append_new(channels, json_string_zen("..."));
						break;
					}
					json_array_append_new(channels, json_string_zen(m->channel->name));
				}
			} else
			{
				...
				for (m = client->user->channel; m; m = m->next)
				{
					...
					json_object_set_new(e, "name", json_string_zen(m->channel->name));
					if (*m->member_modes)
						json_object_set_new(e, "level", json_string_zen(m->member_modes));
	...
	} else if (IsMe(client))
	{
		...
		if (!BadPtr(client->info))
			json_object_set_new(server, "info", json_string_zen(client->info));
		...
		{
			char buf[256];
			snprintf(buf, sizeof(buf), "ZenIRCd-%s", buildid);
			json_object_set_new(features, "software", json_string_zen(buf));
		}
		json_object_set_new(features, "protocol", json_integer(ZenProtocol));
		if (!BadPtr(client->server->features.usermodes))
			json_object_set_new(features, "usermodes", json_string_zen(umodestring));
		...
		/* client.server.features.chanmodes (array) */
		{
			...
			json_array_append_new(chanmodes, json_string_zen(buf));
			for (i = 1; i < 4; i++)
				json_array_append_new(chanmodes, json_string_zen(extchmstr[i]));
		}
		if (!BadPtr(client->server->features.nickchars))
			json_object_set_new(features, "nick_character_sets", json_string_zen(charsys_get_current_languages()));
	...
	} else if (IsServer(client) && client->server)
	{
		...
		if (!BadPtr(client->info))
			json_object_set_new(server, "info", json_string_zen(client->info));
		if (client->uplink)
			json_object_set_new(server, "uplink", json_string_zen(client->uplink->name));
		json_object_set_new(server, "num_users", json_integer(client->server->users));
		json_object_set_new(server, "boot_time", json_timestamp(client->server->boottime));
		...
		json_object_set_new(server, "features", features);
		if (!BadPtr(client->server->features.software))
			json_object_set_new(features, "software", json_string_zen(client->server->features.software));
		json_object_set_new(features, "protocol", json_integer(client->server->features.protocol));
		if (!BadPtr(client->server->features.usermodes))
			json_object_set_new(features, "usermodes", json_string_zen(client->server->features.usermodes));
		if (!BadPtr(client->server->features.chanmodes[0]))
		{
			...
			json_object_set_new(features, "chanmodes", chanmodes);
			for (i = 0; i < 4; i++)
				json_array_append_new(chanmodes, json_string_zen(client->server->features.chanmodes[i]));
		}
		if (!BadPtr(client->server->features.nickchars))
			json_object_set_new(features, "nick_character_sets", json_string_zen(client->server->features.nickchars));
		RunHook(HOOKTYPE_JSON_EXPAND_CLIENT_SERVER, client, detail, child, server);
	}
	RunHook(HOOKTYPE_JSON_EXPAND_CLIENT, client, detail, child);
}
...
void json_expand_channel_ban(json_t *child, const char *banlist_name, Ban *banlist)
{
	...
	for (ban = banlist; ban; ban = ban->next)
	{
		...
		json_object_set_new(e, "name", json_string_zen(ban->banstr));
		json_object_set_new(e, "set_by", json_string_zen(ban->who));
...
void json_expand_channel(json_t *j, const char *key, Channel *channel, int detail)
{
	...
	json_object_set_new(child, "name", json_string_zen(channel->name));
	if (detail == 0)
		return;
	...
	if (channel->topic)
	{
		json_object_set_new(child, "topic", json_string_zen(channel->topic));
		json_object_set_new(child, "topic_set_by", json_string_zen(channel->topic_nick));
	...
	if (*mode2)
	{
		...
		json_object_set_new(child, "modes", json_string_zen(modes));
	} else
	{
		json_object_set_new(child, "modes", json_string_zen(mode1 + 1));
	}
	...
	if (detail >= 3)
	{
		...
		for (u = channel->members; u; u = u->next)
		{
			json_t *e = json_object();
			if (*u->member_modes)
				json_object_set_new(e, "level", json_string_zen(u->member_modes));
...
void json_expand_tkl(json_t *root, const char *key, TKL *tkl, int detail)
{
	...
	json_object_set_new(j, "type", json_string_zen(tkl_type_config_string(tkl))); // Eg 'kline'
	json_object_set_new(j, "type_string", json_string_zen(tkl_type_string(tkl))); // Eg 'Soft K-Line'
	json_object_set_new(j, "set_by", json_string_zen(tkl->set_by));
	if (tkl->id[0])
		json_object_set_new(j, "id", json_string_zen(tkl->id));
	...
	*buf = '\0';
	...
	json_object_set_new(j, "set_at_string", json_string_zen(buf));
	if (tkl->expire_at <= 0)
	{
		json_object_set_new(j, "expire_at_string", json_string_zen("Never"));
		json_object_set_new(j, "duration_string", json_string_zen("permanent"));
	} else
	{
		*buf = '\0';
		short_date(tkl->expire_at, buf);
		strlcat(buf, " GMT", sizeof(buf));
		json_object_set_new(j, "expire_at_string", json_string_zen(buf));
		json_object_set_new(j, "duration_string", json_string_zen(pretty_time_val_r(buf, sizeof(buf), tkl->expire_at - tkl->set_at)));
	}
	...
	if (TKLIsServerBan(tkl))
	{
		if (tkl->ptr.serverban->match)
			...
			json_object_set_new(j, "name", json_string_zen(tkl_uhost(tkl, buf, sizeof(buf), 0)));
		json_object_set_new(j, "reason", json_string_zen(tkl->ptr.serverban->reason));
		if (tkl->spamfilter_id[0])
			json_object_set_new(j, "spamfilter_id", json_string_zen(tkl->spamfilter_id));
	...
	{
		json_object_set_new(j, "name", json_string_zen(tkl->ptr.nameban->name));
		json_object_set_new(j, "reason", json_string_zen(tkl->ptr.nameban->reason));
		json_object_set_new(j, "hits", json_integer(tkl->hits));
		json_object_set_new(j, "last_hit_at", json_timestamp(tkl->lasthit));
	} else if (TKLIsBanException(tkl))
	{
		if (tkl->ptr.banexception->match)
			...
			json_object_set_new(j, "name", json_string_zen(tkl_uhost(tkl, buf, sizeof(buf), 0)));
		json_object_set_new(j, "reason", json_string_zen(tkl->ptr.banexception->reason));
		json_object_set_new(j, "exception_types", json_string_zen(tkl->ptr.banexception->bantypes));
	} else if (TKLIsSpamfilter(tkl))
	{
		if (tkl->ptr.spamfilter->match->str)
		{
			json_object_set_new(j, "name", json_string_zen(tkl->ptr.spamfilter->match->str));
			json_object_set_new(j, "match_type", json_string_zen(zen_match_method_valtostr(tkl->ptr.spamfilter->match->type)));
		}
		if (tkl->ptr.spamfilter->prettyrule)
			json_object_set_new(j, "rule", json_string_zen(tkl->ptr.spamfilter->prettyrule));
		if (tkl->ptr.spamfilter->except)
			json_expand_security_group(j, "match", tkl->ptr.spamfilter->except, 1);
		json_object_set_new(j, "ban_action", json_string_zen(ban_actions_to_string(tkl->ptr.spamfilter->action)));
		json_object_set_new(j, "ban_duration", json_integer(tkl->ptr.spamfilter->tkl_duration));
		json_object_set_new(j, "ban_duration_string", json_string_zen(pretty_time_val_r(buf, sizeof(buf), tkl->ptr.spamfilter->tkl_duration)));
		json_object_set_new(j, "spamfilter_targets", json_string_zen(spamfilter_target_inttostring(tkl->ptr.spamfilter->target)));
		json_object_set_new(j, "reason", json_string_zen(zen_decodespace(tkl->ptr.spamfilter->tkl_reason)));
		json_object_set_new(j, "hits", json_integer(tkl->hits));
		json_object_set_new(j, "last_hit_at", json_timestamp(tkl->lasthit));
...
void json_expand_textanalysis(json_t *root, const char *key, TextAnalysis *ta, int detail)
{
	...
	json_object_set_new(j, "deconfused", json_string_zen(ta->deconfused));
...
void json_expand_mask_list(json_t *parent, const char *key, ConfigItem_mask *mask)
{
	...
	for (m = mask; m; m = m->next)
		json_array_append_new(arr, json_string_zen(m->mask));
}
...
void json_expand_name_list(json_t *parent, const char *key, NameList *list)
{
	...
	for (n = list; n; n = n->next)
		json_array_append_new(arr, json_string_zen(n->name));
}
...
void json_expand_nvplist(json_t *parent, const char *key, NameValuePrioList *list)
{
	...
	for (n = list; n; n = n->next)
		json_object_set_new(obj, n->name, json_string_zen(n->value));
}
...
void json_expand_nvplist_prefix(json_t *parent, const char *prefix, NameValuePrioList *list)
{
	...
	for (n = list; n; n = n->next)
	{
		...
		json_object_set_new(parent, buf, json_string_zen(n->value));
	}
}
...
void json_expand_security_group(json_t *j, const char *key, SecurityGroup *s, int detail)
{
	...
	if (*s->name)
	{
		json_object_set_new(child, "name", json_string_zen(s->name));
	...
	if (s->prettyrule)
		json_object_set_new(child, "rule", json_string_zen(s->prettyrule));
	...
	if (s->exclude_prettyrule)
		json_object_set_new(child, "exclude_rule", json_string_zen(s->exclude_prettyrule));
}
