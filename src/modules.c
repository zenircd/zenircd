/************************************************************************
 *   ZenIRCd - Zen Internet Relay Chat Daemon - src/modules.c
...
void DeleteTempModules(void)
{
	...
	while ((dir = readdir(fd)))
	{
		...
		if (!strstr(fname, ".so") && !strstr(fname, ".conf") &&
		    (strstr(fname, "core") || strstr(fname, "zenircd_asan.")))
 ...
 * something like "/home/xyz/zenircd/modules/third/la.so
 * (and other tricks)
 */
...
const char *Module_Create(const char *path_)
{
	...
	if (loop.config_test)
	{
		/* For './zenircd configtest' we don't have to do any copying and shit */
		tmppath = path;
	} else
	{
		tmppath = zen_mktemp(TMPDIR, zen_getmodfilename(path));
		if (!tmppath)
		 ...
		 */
		ret = zen_copyfileex(path, tmppath, 0);
		if (!ret)
		{
