 ...
 *   (C) 2003-2019 Bram Matthys (Syzop) and the ZenIRCd Team
 *
...
void (*do_zen_log_remote_deliver)(LogLevel loglevel, const char *subsystem, const char *event_id, MultiLine *msg, const char *json_serialized);
...
int (*zen_match_iplist)(Client *client, NameList *l);
...
char *(*zen_expand_string)(const char *str, char *buf, size_t buflen, NameValuePrioList *nvp, int buildvarstring_options, Client *client);
...
void efunctions_init(void)
{
	...
	efunc_init_function(EFUNC_DO_ZEN_LOG_REMOTE_DELIVER, do_zen_log_remote_deliver, do_zen_log_remote_deliver_default_handler, 0);
	...
	efunc_init_function(EFUNC_ZEN_MATCH_IPLIST, zen_match_iplist, NULL, 0);
	...
	efunc_init_function(EFUNC_ZEN_EXPAND_STRING, zen_expand_string, NULL, 0);
