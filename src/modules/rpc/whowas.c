/* whowas.* RPC calls
 * (C) Copyright 2023-.. Bram Matthys (Syzop) and the ZenIRCd team
...
void json_expand_whowas(json_t *j, const char *key, WhoWas *e, int detail)
{
	...
	json_object_set_new(child, "name", json_string_zen(e->name));
	json_object_set_new(child, "event", json_string_zen(whowas_event_to_string(e->event)));
	...
	json_object_set_new(child, "hostname", json_string_zen(e->hostname));
	json_object_set_new(child, "ip", json_string_zen(e->ip));
	...
	json_object_set_new(user, "username", json_string_zen(e->username));
	if (!BadPtr(e->realname))
		json_object_set_new(user, "realname", json_string_zen(e->realname));
	if (!BadPtr(e->virthost))
		json_object_set_new(user, "vhost", json_string_zen(e->virthost));
	json_object_set_new(user, "servername", json_string_zen(e->servername));
	if (!BadPtr(e->account))
		json_object_set_new(user, "account", json_string_zen(e->account));
	...
	if (e->ip)
	{
		...
		if (geo)
		{
			...
			if (geo->country_code)
				json_object_set_new(geoip, "country_code", json_string_zen(geo->country_code));
			if (geo->asn)
				json_object_set_new(geoip, "asn", json_integer(geo->asn));
			if (geo->asname)
				json_object_set_new(geoip, "asname", json_string_zen(geo->asname));
