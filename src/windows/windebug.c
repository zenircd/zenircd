...
LONG __stdcall ExceptionFilter(EXCEPTION_POINTERS *e)
{
	...
	sprintf(file, "zenircd.%d.core", getpid());
	fd = fopen(file, "w");
	...
	sprintf(text, "ZenIRCd has encountered a fatal error. Debugging information has been dumped to %s.", file);
	fclose(fd);
	...
	if (hDll)
	{
		...
		if (pDump)
		{
			...
			sprintf(minidumpf, "zenircd.%d.mdmp", getpid());
			...
			if (hDump != INVALID_HANDLE_VALUE)
			{
				...
				{
					sprintf(text, "ZenIRCd has encountered a fatal error. Debugging information has been dumped to %s and %s.", file, minidumpf);
				}
				CloseHandle(hDump);
			}
			sprintf(minidumpf, "zenircd.%d.full.mdmp", getpid());
			hDump = CreateFile(minidumpf, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
			if (hDump != INVALID_HANDLE_VALUE)
			{
				ExInfo.ThreadId = GetCurrentThreadId();
	...
	} else
	{
		...
		if (fd)
		{
			fprintf(fd, "ZenIRCd has encountered a fatal error. Debugging information "
			            "has been dumped to zenircd.%d.core, please file a bug and upload "
