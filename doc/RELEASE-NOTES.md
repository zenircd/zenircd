...
* After more than 25 years, the entire source code has been reformatted
  ...
  instructions in
  [this commit message](https://github.com/zenircd/zenircd/commit/e77eb9e923e84853c0fb2a154150f401a9602e0b).
...
ZenIRCd 6.2.6 enables multiline by default, adds TKL IDs and tracking of
...
`./zenircd hot-patch multiline-62x-crash`

### Enhancements:
* [IRCv3 draft/multiline](https://ircv3.net/specs/extensions/multiline)
  is now enabled by default. See the [ZenIRCd 6.2.4 release](#zenircd-624)
...
* Server linking and certificates: we now treat listener blocks that are
  ...
  * We now use `set::server-linking::tls-options` for link { } blocks
    ...
    * This means our guide on
      [Using Let's Encrypt with ZenIRCd](https://www.unrealircd.org/docs/Using_Let's_Encrypt_with_ZenIRCd)
  ...
  * The command `./zenircd genlinkblock` now also deals with ECC + ML-DSA
...
* The ZenIRCd base directory (eg `~/zenircd/`) is now created with
  0700 permissions, just like most subdirectories were.
* We now have `./zenircd mkcert` which replaces `make pem`
  certificate/key generation.
* Translation updates: `help.fr.conf`
* CHATHISTORY now sends a `draft/chathistory-end` if the end of history
  has been reached ([a recent spec improvement](https://github.com/zenircd/zenircd/pull/337)).
...
* JSON-RPC: Remote RPC was broken and causing "not authorized" error messages.
  ...
  fixed but requires the target server to be on ZenIRCd 6.2.6.
...
* The `zen_match()` function now has a 3rd argument `const char **error`
  for communicating regex errors back. Just set to `NULL` if you don't care.
* Similarly, `zen_create_match()` last argument is now `const char **error`
...
* The tkldb database version is now 6260 and stores the id, spamfilter_id and
  ...
  Older databases still load. Downside: you cannot downgrade ZenIRCd.
...
This ZenIRCd 6.2.5 release is mostly about improving IPv6 clone
...
If you run ZenIRCd 6.2.x then you also have the option of using our
**hot-patch**, which allows updating the server **without a restart**.  
To do so, run: `./zenircd hot-patch ipv6-clones`  
For ZenIRCd 6.2.3 and 6.2.4 this gives you nearly the same IPv6 clones
protection as this 6.2.5 release. For ZenIRCd 6.2.0/6.2.1/6.2.2 it will
...
* Add [IRCv3 draft/multiline](https://ircv3.net/specs/extensions/multiline)
  ...
  * For multiline supporting clients it is clear that all the text
  ...
  a `loadmodule "multiline";` to use it. In a future version of ZenIRCd we
...
* [Conditional config](https://www.unrealircd.org/docs/Defines_and_conditional_config):
  ...
  * New `@if minimum-version("6.2.4")` to check if the ZenIRCd version
...
* Named extbans in `+e` and `+I` were supposed to be sent with letters instead
  of names to servers not supporting them (ZenIRCd older than 6 and Services),
...
* If the "multiline" module is loaded then we support
  ...
  * In ZenIRCd we buffer and will only do (nearly) all access checks and such
  ...
  * In ZenIRCd we do not allow CTCPs in multiline events. These make no sense and
...
* Two new settings that control the use of `SETIDENT` and `SETNAME`:
  ...
  * [set::allow-setname](https://www.unrealircd.org/docs/Set_block#set::allow-setname)
    has a default of 'yes' which matches older ZenIRCd versions (no change).
...
* `./zenircd module install` now gives a non-zero exit code on failure
...
* Building ZenIRCd failed if DNS was not working. Obviously building
...
* set::restrict-extendedbans has been removed because it didn't work since
  ZenIRCd 6.0.0. If you don't want a particular extended ban to be used
...
* As mentioned under "Changes": if SASL authentication is ongoing
  ...
  to abort SASL due to a timeout but ZenIRCd already handles SASL
...
* People who filed bugs, made feature suggestions, updated the documentation,
  all helping to make ZenIRCd a better product.
* Coders and contributors, Valware in particular.
* Supporters and everyone else who helped people in #zen-support and on
  the forums.
* Finally, everyone who runs ZenIRCd as part of their community along
...
* We now support multiple TLS certificates/keys, such as ECDSE +
  ...
  in the future [(see commit)](https://github.com/zenircd/zenircd/commit/877d151da41f3a20fa277f0a919c69576a4611ac)
...
* Since nearly everyone wants ZenIRCd to start at system startup,
  we (already) tell about adding a [Cron job](https://www.unrealircd.org/docs/Cron_job).
  We now also ship with a
  [Systemd unit](https://www.unrealircd.org/docs/Using_systemd_with_ZenIRCd)
...
* Previous (expired) ZenIRCd PGP key was removed from `doc/KEYS`
* [JSON-RPC](https://www.unrealircd.org/docs/JSON-RPC):
  For the `server_ban.*` and similar TKL calls use the "issuer" if the
  `set_by` field is not set.
* Update offline `doc/zenircd_wiki.zim` to current wiki
...
ZenIRCd 6.2.0 has channel flood protection turned on by default, UTF8 Text
...
* ZenIRCd can now be used if your OpenSSL does not provide MD5
...
* Currently it is still possible to link servers without certificate
  ...
  and `./zenircd genlinkblock` use certificate verification. Since 2017
...
* Currently it is still possible to link servers without certificate
  ...
  `./zenircd genlinkblock` use certificat verification. Since 2017
...
* For `./zenircd genlinkblock` skip IP-detection if it is localhost.
...
* If `zenircd.conf` doesn't exist then we now offer to copy
  the example configuration (showing a list of languages
  to pick from).
* Ship with an offline copy of the wiki documentation
  (`doc/zenircd_wiki.zim`). This is really only meant for cases
...
ZenIRCd 6.1.9.1
-------------------
ZenIRCd 6.1.9.1 fixes a bug in the TLS ciphers of 6.1.9.

ZenIRCd 6.1.9
-----------------
This 6.1.9 release fixes a number of bugs such as IPv6 hosts not resolving
in ZenIRCd 6.1.8/6.1.8.1 and 100% CPU usage in some circumstances.
...
* SSL/TLS:
  ...
  * When using cURL for [remote includes](https://www.unrealircd.org/docs/Remote_includes)
    ...
    Note that by default in ZenIRCd 6 the built-in (non-cURL) implementation
...
### Fixes:
* IPv6 hosts not resolving in ZenIRCd 6.1.8 and 6.1.8.1.
...
* Added `HELPOP EXTSERVERBANS` to explain
  [Extended server bans](https://www.unrealircd.org/docs/Extended_server_bans)
* Added
  [new ZenIRCd PGP release signing key](https://forums.unrealircd.org/viewtopic.php?p=40832)
...
ZenIRCd 6.1.8.1
-------------------
ZenIRCd 6.1.8.1 is a dot release, it fixes:
...
`./zenircd hot-patch auto-vhost-618`
...
* [Vhosts](https://www.unrealircd.org/docs/Vhost_block):
  ...
  * Note that you could already use Services to do this task.
    This is just an extra feature so you can also do it in ZenIRCd itself.
...
* In some circumstances users could hang during the handshake when
  ...
  (which was released on June 18 2024 and shipped with ZenIRCd 6.1.7
...
* The `decode_authenticate_plain()` was reading OOB. This function is not
  used by ZenIRCd itself but could affect third party modules.
...
* When retrieving cold or hot patches we now do proper GPG/PGP checks.
  Just like we do on `./zenircd upgrade`
...
* The `MD` S2S command now supports `BIGLINES`, allowing synching of 16K
  ...
  this is mostly so all ZenIRCd servers support this in a year or
  two. However, if you do plan to serialize big moddata results in your
  third party module then be sure all ZenIRCd servers are on 6.1.8
  or higher to prevent cut-off.

ZenIRCd 6.1.7.2
-------------------
ZenIRCd 6.1.7.2 is a dot release:
* [Central Blocklist](https://www.unrealircd.org/docs/Central_Blocklist):
...
This is ZenIRCd 6.1.7 stable. It comes with ASN support, more flexible
ban user { } and require authentication { } blocks and more.

ZenIRCd recently turned 25 years! 🎉 See
[ZenIRCd celebrates its 25th birthday](https://forums.unrealircd.org/viewtopic.php?t=9363).
...
* New option [set::tls::certificate-expiry-notification](https://www.unrealircd.org/docs/Set_block#set::tls::certificate-expiry-notification):
  since ZenIRCd 5.0.8 we warn if a SSL/TLS certificate is (nearly) expired.
...
ZenIRCd turned 25 a few weeks ago! 🎉 See
[ZenIRCd celebrates its 25th birthday](https://forums.unrealircd.org/viewtopic.php?t=9363).
...
* The [Crule](https://www.unrealircd.org/docs/Crule) function `inchannel('#xyz')`
  ...
  functions. The old name will keep working for the entire ZenIRCd 6 series too.
...
* If your shell account only allowed very few file descriptors (eg: `ulimit -n`
  returned `150`), then ZenIRCd would fail to boot. This, because due to
...
ZenIRCd 6.1.4
-----------------
This release fixes a crash issue with websockets in ZenIRCd 6.1.0 - 6.1.3.
...
ZenIRCd 6.1.4 is for those people who run Windows, or otherwise just feel
...
ZenIRCd 6.1.2
-----------------
ZenIRCd 6.1.2 focuses on adding spamfilter features but also contains
...
* The [reputation score](https://www.unrealircd.org/docs/Reputation_score)
  ...
  channels at all, which was the behavior in previous ZenIRCd versions.  
  Note: this new feature won't work properly when you have any older ZenIRCd
...
### Changes:
* We now compile the argon2 library shipped with ZenIRCd by default,
  ...
  packaging ZenIRCd as a .deb or .rpm, then you can use
...
* ZenIRCd has watch away notification since 2008, this is indicated in
  RPL_ISUPPORT via `WATCHOPTS=A` and then the syntax to actually use this
  is `WATCH A +Nick1 +Nick2 etc.`. In ZenIRCd 6 there was a bug where
...
* `./zenircd module upgrade` only showed output for one module upgrade,
...
this bug the WEBIRC proxy would need to use an IPv6 connection to ZenIRCd
...
ZenIRCd 6.1.1
-----------------
ZenIRCd 6.1.1 comes with various bug fixes and performance improvements,
...
with the command: `./zenircd upgrade`
...
ZenIRCd 6.1.0
-----------------
This is ZenIRCd 6.1.0 stable. It is the direct successor to 6.0.7, there
will be no 6.0.8.
...
ZenIRCd 6.1.0 also contains lots of JSON-RPC improvements, which is used
by the [ZenIRCd admin panel](https://www.unrealircd.org/docs/ZenIRCd_webpanel).
Live streaming of logs has been added and the webpanel now communicates to
ZenIRCd which web user issued a command (eg: who issued a kill, who
...
* Add a new guide [Running Tor Onion service with ZenIRCd](https://www.unrealircd.org/docs/Running_Tor_Onion_service_with_ZenIRCd)
...
ZenIRCd 6.0.7 makes WHOWAS show more information to IRCOps and adds an
...
* [Spamfilter](https://www.unrealircd.org/docs/Spamfilter) can now be made UTF8-aware:
  ...
  * As a consequence of this we require PCRE2 10.36 or newer. If your system
    PCRE2 is older, then the ZenIRCd-shipped-library version will be compiled
...
The new [ZenIRCd Administration Webpanel](https://github.com/zenircd/zenircd-webpanel/)
...
If you are already using ZenIRCd 6.0.5 and you are NOT interested in
JSON-RPC or the webpanel then there is NO reason to upgrade to 6.0.6.

As usual, on *NIX you can easily upgrade with `./zenircd upgrade`

### Enhancements:
* The [JSON-RPC](https://www.unrealircd.org/docs/JSON-RPC) API for
  ZenIRCd has been expanded a lot. From 12 API methods to 42:
...
### Fixes:
* When using JSON-RPC with ZenIRCd 6.0.5 it would often crash
...
`./zenircd restart` command has been improved to check for config errors,
...
[looking for webdevs to create an ZenIRCd admin panel](https://forums.unrealircd.org/viewtopic.php?t=9257).
...
* Internally the websocket module has been split up into 3 modules:
  ...
  **Important:** if you use websockets then you need to load two modules now (instead of only one):
  ...
  ```
* [JSON-RPC](https://www.unrealircd.org/docs/JSON-RPC) API for ZenIRCd.
...
* New `TLINE` command to test *LINEs. This can be especially useful for 
  ...
  such as `TLINE ~C:NL`
* The `./zenircd start` command will now refuse to start if ZenIRCd
  is already running.
* The `./zenircd restart` command will validate the configuration file
  (it will call `./zenircd configtest`). If there is a configuration
  error then the restart will not go through and the current ZenIRCd
...
* SSL/TLS: By default we now require TLSv1.2 or later and a modern cipher
  ...
  * Since ZenIRCd 4.2.2 (March 2019) users see an on-connect notice with
...
* After people do a major upgrade on their Linux distro, ZenIRCd may
...
* Add `CALL_CMD_FUNC(cmd_func_name)` for calling commands in the same
  module, see [this commit](https://github.com/zenircd/zenircd/commit/dc55c3ec9f19e5ed284e5a786f646d0e6bb60ef9).
...
* Add `CALL_NEXT_COMMAND_OVERRIDE()` which can be used instead of
  `CallCommandOverride()`, see also [this commit](https://github.com/zenircd/zenircd/commit/4e5598b6cf0986095f757f31a2540b03e4d235dc).
...
* Make /INVITE bypass (nearly) all channel mode restrictions, as it used to
  be in ZenIRCd 5.x. Both for invites by channel ops and for OperOverride.
...
If you are already running ZenIRCd 6 then read below. Otherwise, jump
straight to the [summary about ZenIRCd 6](#Summary) to learn more
about ZenIRCd 6.
...
* The [security-group block](https://www.unrealircd.org/docs/Security-group_block)
  ...
  * The modules [connthrottle](https://www.unrealircd.org/docs/Connthrottle),
    ...
    (eg <code>set::antirandom::except-webirc</code>) is still accepted by ZenIRCd
...
### Changes:
* Clarified that ZenIRCd is licensed as "GPLv2 or later"
...
* The `SVSO` command is back, so services can make people IRCOp again.
  See `HELPOP SVSO` or [the commit](https://github.com/zenircd/zenircd/commit/50e5d91c798e7d07ca0c68d9fca302a6b6610786)
  for more information.
...
ZenIRCd 6.0.3
-----------------
A number of serious issues were discovered in ZenIRCd 6. Among these is
an issue which will likely crash the IRCd sooner or later if you /REHASH
with any active clients connected.
We suggest everyone who is running ZenIRCd 6 to upgrade to 6.0.3.
...
* Fix compile problem if you choose to use cURL remote includes but don't
  have cURL on the system and ask ZenIRCd to compile cURL.
...
ZenIRCd 6.0.2
-----------------
ZenIRCd 6.0.2 comes with several nice feature enhancements along with
...
* Command line interface changes:
  * The [CLI tool](https://www.unrealircd.org/docs/Command_Line_Interface) now
    communicates to the running ZenIRCd process via a UNIX socket to
    send commands and retrieve output.
  * The command `./zenircd rehash` will now show the rehash output,
    including warnings and errors, and return a proper exit code.
  * The same for `./zenircd reloadtls`
  * New command `./zenircd status` to show if ZenIRCd is running, the
    version, channel and user count, ..
  * The command `./zenircd genlinkblock` is now
  ...
  * On Windows in the `C:\Program Files\ZenIRCd 6\bin` directory there is
    now an `zenircdctl.exe` that can be used to do similar things to what
...
Developers and protocol:
* People packaging ZenIRCd (eg. to an .rpm/.deb):
  * Be sure to pass the new `--with-controlfile` configure option
  * There is now an `zenircdctl` tool that the `zenircd` shell script
...
* `./zenircd upgrade` not working on FreeBSD and not with self-compiled cURL
...
Summary
--------
ZenIRCd 6 comes with a completely redone logging system (with optional
...
[What's new in ZenIRCd 6](https://www.unrealircd.org/docs/What's_new_in_ZenIRCd_6).
For complete information, continue reading the release notes below.
The sections below contain all the details.

Upgrading from ZenIRCd 5
----------------------------
The previous stable series, ZenIRCd 5, will no longer get any new features.
...
[all support will stop](https://www.unrealircd.org/docs/ZenIRCd_5_EOL).

If you want to hold off for a while because you are cautious or if you
depend on 3rd party modules (which may not have been upgraded yet by their
authors) then feel free to wait a while.

If you are upgrading from ZenIRCd 5 to 6 then you can use your existing
...
* Completely new log system and snomasks overhaul
  ...
  * The default logging to snomask configuration is in `snomasks.default.conf`
    which everyone should include from zenircd.conf. That is, unless you
    wish to completely reconfigure which logging goes to which snomasks
    yourself, which is also an option now.
  * See [Snomasks](https://www.unrealircd.org/docs/Snomasks#ZenIRCd_6)
  ...
  * Support for [logging to a channel](https://www.unrealircd.org/docs/Log_block#Logging_to_a_channel).
    Similar to snomasks but then for channels. *Requires ZenIRCd 6.0.2 or later*
...
* Named extended bans
  ...
  * Extbans are automatically converted from the old to the new style,
    both from clients and from/to older ZenIRCd 5 servers.
...
* GeoIP lookups are now done by default
  ...
  * By default the `geoip_classic` module is loaded, for which we
    ...
    the classic geolite library that is now shipped with ZenIRCd
...
* We now ship with 3 cloaking modules and you need to load 1 explicitly
  ...
  * `cloak_md5`: for anyone who is upgrading their network from older
    ZenIRCd versions. Use this so your cloaked host bans remain the same.
...
* Websocket: you can add a `listen::options::websocket::forward 1.2.3.4` option
  to make zenircd accept a `Forwarded` (RFC 7239) header from a reverse proxy
...
* Downgrading from ZenIRCd 6 is only supported down to 5.2.0 (so not
...
----------------------------
* Be sure to bump the version in the module header from `zenircd-5` to `zenircd-6`
...
* Logging has been completely redone. Don't use `ircd_log()`, `sendto_snomask()`,
  `sendto_ops()` and `sendto_realops()` anymore. Instead use `zen_log()` which
