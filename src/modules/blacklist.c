 ...
 * (C) Copyright 2015-.. Bram Matthys (Syzop) and the ZenIRCd team
 *
...
int blacklist_dns_request(Client *client, Blacklist *d)
{
	...
	zen_gethostbyname_api(buf, AF_INET, "blacklist_resolver_callback", BLUSER(client));
...
void blacklist_hit(Client *client, Blacklist *bl, int reply)
{
	...
	zen_expand_string(bl->reason, banbuf, sizeof(banbuf), nvp, 0, client);
