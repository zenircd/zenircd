/************************************************************************
 *   Zen Internet Relay Chat Daemon, src/ircd.c
...
EVENT(garbage_collect)
{
	...
	if (loop.do_garbage_collect == 1)
		zen_log(ULOG_INFO, "main", "GARBAGE_COLLECT_STARTED", NULL, "Doing garbage collection...");
...
static int bad_command(const char *argv0)
{
	...
	printf("ERROR: Incorrect command line argument encountered (if you are looking for foreground mode, it is -F).\n"
	       "IMPORTANT: This is the zenircd BINARY. End-users should NOT call this binary directly.\n"
	       "Please run the SCRIPT instead: %s/zenircd\n",
	       SCRIPTDIR);
...
void SetupEvents(void)
{
	...
	EventAdd(NULL, "zendns_removeoldrecords", zendns_removeoldrecords, NULL, 15000, 0);
	...
	EventAdd(NULL, "zendb_expire_secret_cache", zendb_expire_secret_cache, NULL, 61000, 0);
...
#else
int InitZenIRCd(int argc, char *argv[])
#endif
{
#ifndef _WIN32
	uid_t uid, euid;
...
#ifdef _WIN32
	CreateMutex(NULL, FALSE, "ZenMutex");
	...
	if (euid == 0)
	{
		fprintf(stderr,
		        "** ERROR **\n"
		        "You attempted to run ZenIRCd as root. This is VERY DANGEROUS\n"
		        "as any compromise of your ZenIRCd will result in full\n"
		        "privileges to the attacker on the entire machine.\n"
		        "You MUST start ZenIRCd as a different user!\n"
		        "\n"
...
#endif
#ifndef _WIN32
	fprintf(stderr, "%s", zenlogo);
	fprintf(stderr, "                           v%s\n\n", VERSIONONLY);
	fprintf(stderr, "ZenIRCd is brought to you by Bram Matthys (Syzop),\n"
	...
	if (loop.config_test)
	{
		zen_log(ULOG_INFO, "config", "CONFIG_PASSED", NULL, "Configuration test passed OK");
		fflush(stderr);
		exit(0);
	}
...
#endif
	config_run();
	zen_log(ULOG_INFO, "main", "ZENIRCD_START", NULL, "ZenIRCd started.");
	...
	me.server->features.protocol = ZenProtocol;
...
void SocketLoop(void *dummy)
{
	...
	while (1)
	{
		...
		if (doreloadcert)
		{
			zen_log(ULOG_INFO, "config", "CONFIG_RELOAD_TLS", NULL, "Reloading all TLS related data (./zenircd reloadtls)");
			reinit_tls();
			doreloadcert = 0;
		}
