 ...
 *   (C) 2003 Dominick Meglio and the ZenIRCd Team
...
static void set_curl_tls_options(CURL *curl, int minimum_tls_version)
{
	...
	curl_easy_setopt(curl, CURLOPT_SSL_CIPHER_LIST, ZENIRCD_DEFAULT_CIPHERS);
#endif
#if LIBCURL_VERSION_NUM >= 0x073d00
	/* Set cipher list for TLSv1.3 (cURL 7.61.0+) */
	curl_easy_setopt(curl, CURLOPT_TLS13_CIPHERS, ZENIRCD_DEFAULT_CIPHERSUITES);
#endif
	/* TODO: ideally we would set ZENIRCD_DEFAULT_TLS_GROUPS_PRIMARY,
	 ...
	 * Also can't assume that if it is working in ZenIRCd (which uses openssl)
	 * it will work with curl (which may use gnutls).
	 * So yeah, opted not to set it.
	 * In url_zen we control everything and we do.
	 */
}
...
{
	// DUPLICATE CODE: same as src/url_zen.c, well.. sortof
	Download *handle = (Download *)stream;
	size_t write_sz = size * nmemb;
			...
			if (msg->data.result == CURLE_OK)
			{
				...
				} else
				{
					if ((last_mod != -1) && handle->filename)
						zen_setfilemodtime(handle->filename, last_mod);
...
void url_start_async(OutgoingWebRequest *request)
{
	...
	snprintf(user_agent, sizeof(user_agent), "ZenIRCd %s", VERSIONONLY);
