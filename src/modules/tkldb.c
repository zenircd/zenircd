 ...
 * (C) Copyright 2019 Gottem and the ZenIRCd team
 *
...
#define WARN_WRITE_ERROR(fname) \
	do \
	{ \
		zen_log(ULOG_ERROR, "tkldb", "TKLDB_FILE_WRITE_ERROR", NULL, \
...
#define R_SAFE(x) \
	...
	{ \
		if (!(x)) \
		{ \
			config_warn("[tkldb] Read error from database file '%s' (possible corruption): %s", cfg.database, zendb_get_error_string()); \
			zendb_close(db); \
			FreeTKLRead(); \
			return 0; \
		} \
...
int write_tkline(ZenDB *db, const char *tmpfname, TKL *tkl);
int read_tkldb(void);
...
int tkldb_config_test(ConfigFile *cf, ConfigEntry *ce, int type, int *errs)
{
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
		...
		{
			const char *err;
			if ((err = zendb_test_secret(cep->value)))
			{
...
int tkldb_config_posttest(int *errs)
{
	...
	if (test.database && ((errstr = zendb_test_db(test.database, test.db_secret))))
	{
...
int write_tkldb(void)
{
	...
	db = zendb_open(tmpfname, ZENDB_MODE_WRITE, cfg.db_secret);
	if (!db)
	{
		WARN_WRITE_ERROR(tmpfname);
		return 0;
	...
	W_SAFE(zendb_write_int32(db, TKLDB_MAGIC));
	W_SAFE(zendb_write_int32(db, tkldb_version));
	...
	W_SAFE(zendb_write_int64(db, tklcount));
...
int write_tkline(ZenDB *db, const char *tmpfname, TKL *tkl)
{
	...
	W_SAFE(zendb_write_char(db, tkltype)); // TKL char

	W_SAFE(zendb_write_str(db, tkl->set_by));
	W_SAFE(zendb_write_int64(db, tkl->set_at));
	W_SAFE(zendb_write_int64(db, tkl->expire_at));
	W_SAFE(zendb_write_str(db, tkl->id));     /* since TKLDB_VERSION 6260 */
	W_SAFE(zendb_write_str(db, tkl->spamfilter_id)); /* since TKLDB_VERSION 6260 */

	/* Hit-stat fields for all TKL types (since TKLDB_VERSION 6260): hits, lasthit. */
	W_SAFE(zendb_write_int64(db, tkl->hits));
	W_SAFE(zendb_write_int64(db, tkl->lasthit));
	...
	if (TKLIsServerBan(tkl))
	{
		...
		W_SAFE(zendb_write_str(db, usermask));
		W_SAFE(zendb_write_str(db, tkl->ptr.serverban->hostmask));
		W_SAFE(zendb_write_str(db, tkl->ptr.serverban->reason));
	} else if (TKLIsBanException(tkl))
	{
		...
		W_SAFE(zendb_write_str(db, usermask));
		W_SAFE(zendb_write_str(db, tkl->ptr.banexception->hostmask));
		W_SAFE(zendb_write_str(db, tkl->ptr.banexception->bantypes));
		W_SAFE(zendb_write_str(db, tkl->ptr.banexception->reason));
	} else if (TKLIsNameBan(tkl))
	{
		char *hold = tkl->ptr.nameban->hold ? "H" : "*";
		W_SAFE(zendb_write_str(db, hold));
		W_SAFE(zendb_write_str(db, tkl->ptr.nameban->name));
		W_SAFE(zendb_write_str(db, tkl->ptr.nameban->reason));
	} else if (TKLIsSpamfilter(tkl))
	{
		char *match_type = zen_match_method_valtostr(tkl->ptr.spamfilter->match->type);
		...
		W_SAFE(zendb_write_str(db, match_type));
		W_SAFE(zendb_write_str(db, tkl->ptr.spamfilter->match->str));
		W_SAFE(zendb_write_str(db, target));
		W_SAFE(zendb_write_char(db, action));
		W_SAFE(zendb_write_str(db, tkl->ptr.spamfilter->tkl_reason));
		W_SAFE(zendb_write_int64(db, tkl->ptr.spamfilter->tkl_duration));
		/* Spamfilter-only hit-stat fields (since TKLDB_VERSION 6260): hits_except, lasthit_except. */
		W_SAFE(zendb_write_int64(db, tkl->ptr.spamfilter->hits_except));
		W_SAFE(zendb_write_int64(db, tkl->ptr.spamfilter->lasthit_except));
	}
...
int read_tkldb(void)
{
	...
	db = zendb_open(cfg.database, ZENDB_MODE_READ, cfg.db_secret);
	if (!db)
	{
		if (zendb_get_error_code() == ZENDB_ERROR_FILENOTFOUND)
		{
			/* Database does not exist. Could be first boot */
			config_warn("[tkldb] No database present at '%s', will start a new one", cfg.database);
			return 1;
		} else if (zendb_get_error_code() == ZENDB_ERROR_NOTCRYPTED)
		{
			/* Re-open as unencrypted */
			db = zendb_open(cfg.database, ZENDB_MODE_READ, NULL);
			if (!db)
			{
				/* This should actually never happen, unless some weird I/O error */
				config_warn("[tkldb] Unable to open the database file '%s': %s", cfg.database, zendb_get_error_string());
				return 0;
			}
		} else
		{
			config_warn("[tkldb] Unable to open the database file '%s' for reading: %s", cfg.database, zendb_get_error_string());
			return 0;
		}
	}

	/* The database starts with a "magic value" - unless it's some old version or corrupt */
	R_SAFE(zendb_read_int32(db, &magic));
	if (magic != TKLDB_MAGIC)
	{
		config_warn("[tkldb] Database '%s' uses an old and unsupported format OR is corrupt", cfg.database);
		config_status("If you are upgrading from ZenIRCd 4 (or 5.0.0-alpha1) then we suggest you to "
	...
	R_SAFE(zendb_read_int32(db, &version));
	if (version < 4999)
	{
	...
	{
		config_warn("[tkldb] Database '%s' has version %lu while we only support %lu. Did you just downgrade ZenIRCd? Sorry this is not suported",
		            cfg.database, (unsigned long)tkldb_version, (unsigned long)version);
		zendb_close(db);
		return 0;
	}

	R_SAFE(zendb_read_int64(db, &tklcount));
	...
	for (cnt = 0; cnt < tklcount; cnt++)
	{
		...
		if (!tkl->type)
		{
			 ...
			 * in core ZenIRCd. In which case we should add some skipping code
		...
		R_SAFE(zendb_read_str(db, &tkl->set_by));
		R_SAFE(zendb_read_int64(db, &v));
		tkl->set_at = v;
		R_SAFE(zendb_read_int64(db, &v));
		tkl->expire_at = v;
		...
		if (TKLIsServerBan(tkl))
		{
			...
			R_SAFE(zendb_read_str(db, &tkl->ptr.serverban->hostmask));
			R_SAFE(zendb_read_str(db, &tkl->ptr.serverban->reason));
		...
		} else if (TKLIsBanException(tkl))
		{
			...
			R_SAFE(zendb_read_str(db, &tkl->ptr.banexception->hostmask));
			R_SAFE(zendb_read_str(db, &tkl->ptr.banexception->bantypes));
			R_SAFE(zendb_read_str(db, &tkl->ptr.banexception->reason));
		...
		} else if (TKLIsNameBan(tkl))
		{
			...
			safe_free(str);
			R_SAFE(zendb_read_str(db, &tkl->ptr.nameban->name));
			R_SAFE(zendb_read_str(db, &tkl->ptr.nameban->reason));
		...
		} else if (TKLIsSpamfilter(tkl))
		{
			...
			/* Match method */
			R_SAFE(zendb_read_str(db, &str));
			match_method = zen_match_method_strtoval(str);
			if (!match_method)
			{
			...
			R_SAFE(zendb_read_str(db, &str));
			tkl->ptr.spamfilter->match = zen_create_match(match_method, str, &err);
			...
			R_SAFE(zendb_read_str(db, &tkl->ptr.spamfilter->tkl_reason));
			R_SAFE(zendb_read_int64(db, &v));
...
#ifdef BENCHMARK
	gettimeofday(&tv_beta, NULL);
	zen_log(ULOG_DEBUG, "tkldb", "TKLDB_BENCHMARK", NULL,
