...
/** @file
 * @brief ZenIRCd database API - see @ref ZenDBFunctions
 */
 ...
 * - File format: https://www.unrealircd.org/docs/Dev:ZenDB
 ...
 * @defgroup ZenDBFunctions Database functions
 */
...
/* In ZenIRCd 5.2.x we didn't write the v1 header yet for unencrypted
 ...
 * Starting with ZenIRCd 6 we now write the header, so people can only
 * downgrade from ZenIRCd 6 to 5.2.0 and later (not 5.0.9).
 */
#define ZENDB_WRITE_V1

/* If a key is specified, it must be this size */
#define ZENDB_KEY_LEN crypto_secretstream_xchacha20poly1305_KEYBYTES

/** Default 'time cost' for Argon2id */
#define ZENDB_ARGON2_DEFAULT_TIME_COST 4
/** Default 'memory cost' for Argon2id. Note that 15 means 1<<15=32M */
#define ZENDB_ARGON2_DEFAULT_MEMORY_COST 15
/** Default 'parallelism cost' for Argon2id. */
#define ZENDB_ARGON2_DEFAULT_PARALLELISM_COST 2
...
static SecretCache *find_secret_cache(Secret *secr, ZenDBConfig *cfg);
static void zendb_add_to_secret_cache(Secret *secr, ZenDBConfig *cfg);
static void zendb_set_error(ZenDB *c, ZenDBError errcode, FORMAT_STRING(const char *pattern), ...) __attribute__((format(printf, 3, 4)));

ZenDBError zendb_last_error_code;
static char *zendb_last_error_string = NULL;

/** Set error condition on zendb 'c' (internal function).
 * @param c		The zendb file handle
 ...
 */
static void zendb_set_error(ZenDB *c, ZenDBError errcode, FORMAT_STRING(const char *pattern), ...)
{
	va_list vl;
	char buf[512];
	va_start(vl, pattern);
	...
	zendb_last_error_code = errcode;
	safe_strdup(zendb_last_error_string, buf);
}

/** Free a ZenDB struct (internal function). */
static void zendb_free(ZenDB *c)
{
	zendb_free_config(c->config);
	safe_free(c->error_string);
	safe_free_sensitive(c);
}

static int zendb_kdf(ZenDB *c, Secret *secr)
{
	if (c->config->kdf != ZENDB_KDF_ARGON2ID)
	{
		zendb_set_error(c, ZENDB_ERROR_INTERNAL, "Unknown KDF 0x%x", (int)c->config->kdf);
		return 0;
	}
	                      ...
	                      c->config->key, c->config->keylen) != ARGON2_OK)
	{
		...
		zendb_set_error(c, ZENDB_ERROR_INTERNAL, "Could not generate argon2 hash - out of memory or something weird?");
		return 0;
	}
	return 1;
}
...
/**
 * @addtogroup ZenDBFunctions
 * @{
 */

/** Get the error string for last failed zendb operation.
 * @returns The error string
 * @note Use the return value only for displaying of errors
 *       to the end-user.
 *       For programmatically checking of error conditions
 *       use zendb_get_error_code() instead.
 */
const char *zendb_get_error_string(void)
{
	return zendb_last_error_string;
}

/** Get the error code for last failed zendb operation
 * @returns An ZEN_DB_ERROR_*
 */
ZenDBError zendb_get_error_code(void)
{
	return zendb_last_error_code;
}

/** Open an zendb file.
 * @param filename	The filename to open
 * @param mode		Either ZENDB_MODE_READ or ZENDB_MODE_WRITE
 * @param secret_block	The name of the secret xx { } block (so NOT the actual password!!)
 * @returns A pointer to a ZenDB structure that can be used in subsequent calls for db read/writes,
 *          and finally zendb_close(). Or NULL in case of failure.
 * @note Upon error (NULL return value) you can call zendb_get_error_code() and
 *       zendb_get_error_string() to see the actual error.
 */
ZenDB *zendb_open(const char *filename, ZenDBMode mode, char *secret_block)
{
	ZenDB *c = safe_alloc_sensitive(sizeof(ZenDB));
	...
	if ((mode != ZENDB_MODE_READ) && (mode != ZENDB_MODE_WRITE))
	{
		zendb_set_error(c, ZENDB_ERROR_API, "zendb_open request for neither read nor write");
	...
	if (secret_block != NULL)
	{
		...
		if (!secr)
		{
			zendb_set_error(c, ZENDB_ERROR_SECRET, "Secret block '%s' not found or invalid", secret_block);
			goto zendb_open_fail;
		}

		if (!valid_secret_password(secr->password, &err))
		{
			zendb_set_error(c, ZENDB_ERROR_SECRET, "Password in secret block '%s' does not meet complexity requirements", secr->name);
			goto zendb_open_fail;
		}
	}

	c->mode = mode;
	c->fd = fopen(filename, (c->mode == ZENDB_MODE_WRITE) ? "wb" : "rb");
	if (!c->fd)
	{
		if (errno == ENOENT)
			zendb_set_error(c, ZENDB_ERROR_FILENOTFOUND, "File not found: %s", strerror(errno));
		else
			zendb_set_error(c, ZENDB_ERROR_IO, "Could not open file: %s", strerror(errno));
		goto zendb_open_fail;
	}

	if (secret_block == NULL)
	{
		if (mode == ZENDB_MODE_READ)
		{
			/* READ: read header, if any, lots of fallback options here... */
			if (fgets(buf, sizeof(buf), c->fd))
			{
				if (str_starts_with_case_sensitive(buf, "ZenIRCd-DB-Crypted"))
				{
					zendb_set_error(c, ZENDB_ERROR_CRYPTED, "file is encrypted but no password provided");
					goto zendb_open_fail;
				} else if (!strcmp(buf, "ZenIRCd-DB-v1"))
				{
					/* Skip over the 32 byte header, directly to the creationtime */
					if (fseek(c->fd, 32L, SEEK_SET) < 0)
					{
						zendb_set_error(c, ZENDB_ERROR_HEADER, "file header too short");
						goto zendb_open_fail;
					}
					if (!zendb_read_int64(c, &c->creationtime))
					{
						zendb_set_error(c, ZENDB_ERROR_HEADER, "Header is too short (A4)");
						goto zendb_open_fail;
					}
					/* SUCCESS = fallthrough */
				} else if (str_starts_with_case_sensitive(buf, "ZenIRCd-DB"))
				{
		...
		{
#ifdef ZENDB_WRITE_V1
			/* WRITE */
			memset(buf, 0, sizeof(buf));
			snprintf(buf, sizeof(buf), "ZenIRCd-DB-v1");
			if ((fwrite(buf, 1, sizeof(buf), c->fd) != sizeof(buf)) ||
			    !zendb_write_int64(c, TStime()))
			{
				zendb_set_error(c, ZENDB_ERROR_IO, "Unable to write header (A1)");
				goto zendb_open_fail;
			}
#endif
		}
		safe_free(zendb_last_error_string);
		zendb_last_error_code = ZENDB_ERROR_SUCCESS;
		return c;
	}

	c->crypted = 1;

	if (c->mode == ZENDB_MODE_WRITE)
	{
		/* Write the:
		 * - generic header ("ZenIRCd-DB" + some zeroes)
		 * - the salt
		 * - the crypto header
		 */
		memset(buf, 0, sizeof(buf));
		snprintf(buf, sizeof(buf), "ZenIRCd-DB-Crypted-v1");
		if (fwrite(buf, 1, sizeof(buf), c->fd) != sizeof(buf))
		{
			zendb_set_error(c, ZENDB_ERROR_IO, "Unable to write header (1)");
			goto zendb_open_fail; /* Unable to write header nr 1 */
		}

		if (secr->cache && secr->cache->config)
		{
			/* Use first found cached config for this secret */
			c->config = zendb_copy_config(secr->cache->config);
			cached = 1;
		} else
		{
			/* Create a new config */
			c->config = safe_alloc(sizeof(ZenDBConfig));
			c->config->kdf = ZENDB_KDF_ARGON2ID;
			c->config->t_cost = ZENDB_ARGON2_DEFAULT_TIME_COST;
			c->config->m_cost = ZENDB_ARGON2_DEFAULT_MEMORY_COST;
			c->config->p_cost = ZENDB_ARGON2_DEFAULT_PARALLELISM_COST;
			c->config->saltlen = ZENDB_SALT_LEN;
			c->config->salt = safe_alloc(c->config->saltlen);
			randombytes_buf(c->config->salt, c->config->saltlen);
			c->config->cipher = ZENDB_CIPHER_XCHACHA20;
			c->config->keylen = ZENDB_KEY_LEN;
		...
		{
			zendb_set_error(c, ZENDB_ERROR_IO, "Unable to write header (2)");
			goto zendb_open_fail;
		}
		...
		{
			zendb_set_error(c, ZENDB_ERROR_IO, "Unable to write header (3)");
			goto zendb_open_fail; /* Unable to write crypto header */
		}
		if (!zendb_write_str(c, "ZenIRCd-DB-Crypted-Now") ||
		    !zendb_write_int64(c, TStime()))
		{
			/* error is already set by zendb_write_str() */
			goto zendb_open_fail; /* Unable to write crypto header */
		}
		if (!cached)
			zendb_add_to_secret_cache(secr, c->config);
	} else
	{
		char *validate = NULL;
		...
		{
			zendb_set_error(c, ZENDB_ERROR_NOTCRYPTED, "Not a crypted file (file too small)");
			goto zendb_open_fail; /* Header too short */
		}
		if (strncmp(buf, "ZenIRCd-DB-Crypted-v1", 24))
		{
			zendb_set_error(c, ZENDB_ERROR_NOTCRYPTED, "Not a crypted file");
			goto zendb_open_fail; /* Invalid header */
		}
		c->config = safe_alloc(sizeof(ZenDBConfig));
		...
		{
			zendb_set_error(c, ZENDB_ERROR_HEADER, "Header is corrupt/unknown/invalid");
			goto zendb_open_fail;
		}
		if (c->config->kdf != ZENDB_KDF_ARGON2ID)
		{
			zendb_set_error(c, ZENDB_ERROR_HEADER, "Header contains unknown KDF 0x%x", (int)c->config->kdf);
			goto zendb_open_fail;
		}
		if (c->config->saltlen > 1024)
		{
			zendb_set_error(c, ZENDB_ERROR_HEADER, "Header is corrupt (saltlen=%d)", (int)c->config->saltlen);
			goto zendb_open_fail; /* Something must be wrong, this makes no sense. */
		}
		c->config->salt = safe_alloc(c->config->saltlen);
		if (fread(c->config->salt, 1, c->config->saltlen, c->fd) != c->config->saltlen)
		{
			zendb_set_error(c, ZENDB_ERROR_HEADER, "Header is too short (2)");
			goto zendb_open_fail; /* Header too short (read II) */
		}
		...
		{
			zendb_set_error(c, ZENDB_ERROR_HEADER, "Header is corrupt/unknown/invalid (3)");
			goto zendb_open_fail;
		}
		if (c->config->cipher != ZENDB_CIPHER_XCHACHA20)
		{
			zendb_set_error(c, ZENDB_ERROR_HEADER, "Header contains unknown cipher 0x%x", (int)c->config->cipher);
			goto zendb_open_fail;
		}
		if (c->config->keylen > 1024)
		{
			zendb_set_error(c, ZENDB_ERROR_HEADER, "Header is corrupt (keylen=%d)", (int)c->config->keylen);
			goto zendb_open_fail; /* Something must be wrong, this makes no sense. */
		}
		c->config->key = safe_alloc_sensitive(c->config->keylen);
		...
		{
			zendb_set_error(c, ZENDB_ERROR_HEADER, "Header is too short (3)");
			goto zendb_open_fail; /* Header too short */
		}
		if (crypto_secretstream_xchacha20poly1305_init_pull(&c->st, header, c->config->key) != 0)
		{
			zendb_set_error(c, ZENDB_ERROR_PASSWORD, "Crypto error - invalid password or corrupt file");
			goto zendb_open_fail; /* Unusual */
		}
		/* Now to validate the key we read a simple string */
		if (!zendb_read_str(c, &validate))
		{
			zendb_set_error(c, ZENDB_ERROR_PASSWORD, "Invalid password");
			goto zendb_open_fail; /* Incorrect key, probably */
		}
		if (strcmp(validate, "ZenIRCd-DB-Crypted-Now"))
		{
			safe_free(validate);
			zendb_set_error(c, ZENDB_ERROR_PASSWORD, "Invalid password");
			goto zendb_open_fail; /* Incorrect key, probably */
		...
		if (!zendb_read_int64(c, &c->creationtime))
		{
			zendb_set_error(c, ZENDB_ERROR_HEADER, "Header is too short (4)");
			goto zendb_open_fail;
		}
		zendb_add_to_secret_cache(secr, c->config);
	}
...
/** Close an zendb file.
 * @param c	The struct pointing to an zendb file
...
int zendb_close(ZenDB *c)
{
	 ...
	 */
	if (c->crypted && (c->mode == ZENDB_MODE_WRITE))
	{
		char buf_out[ZENDB_CRYPT_FILE_CHUNK_SIZE + crypto_secretstream_xchacha20poly1305_ABYTES];
		...
		if (out_len > 0)
		{
			if (fwrite(buf_out, 1, out_len, c->fd) != out_len)
			{
				...
				zendb_set_error(c, ZENDB_ERROR_IO, "Write error: %s", strerror(errno));
				fclose(c->fd);
				zendb_free(c);
				return 0;
			}
 ...
 */
char *zendb_test_db(const char *filename, char *secret_block)
{
	static char buf[512];
	ZenDB *db = zendb_open(filename, ZENDB_MODE_READ, secret_block);
	if (!db)
	{
		if (zendb_get_error_code() == ZENDB_ERROR_PASSWORD)
		{
			snprintf(buf, sizeof(buf), "Incorrect password specified in secret block '%s' for file %s",
			         secret_block, filename);
			return buf;
		}
		if (zendb_get_error_code() == ZENDB_ERROR_CRYPTED)
		{
 ...
 *       zendb_write_int64(), zendb_write_int32(), zendb_write_int16(),
 *       zendb_write_char(), zendb_write_str().
 */
static int zendb_write(ZenDB *c, const void *wbuf, int len)
{
	char buf_out[ZENDB_CRYPT_FILE_CHUNK_SIZE + crypto_secretstream_xchacha20poly1305_ABYTES];
	...
	if (c->mode != ZENDB_MODE_WRITE)
	{
		zendb_set_error(c, ZENDB_ERROR_API, "Write operation requested on a file opened for reading");
		return 0;
	}
	...
	do
	{
		if (c->buflen + len < ZENDB_CRYPT_FILE_CHUNK_SIZE)
		{
			 ...
			 * ZENDB_CRYPT_FILE_CHUNK_SIZE, a common case)
			 */
		...
		{
			/* Fill up c->buf with ZENDB_CRYPT_FILE_CHUNK_SIZE
			 * Note that 'av_bytes' can be 0 here if c->buflen
			 * happens to be exactly ZENDB_CRYPT_FILE_CHUNK_SIZE,
			 * that's okay.
			 */
			int av_bytes = ZENDB_CRYPT_FILE_CHUNK_SIZE - c->buflen;
			if (av_bytes > 0)
				memcpy(c->buf + c->buflen, buf, av_bytes);
			buf += av_bytes;
			len -= av_bytes;
		}
		if (crypto_secretstream_xchacha20poly1305_push(&c->st, buf_out, &out_len, c->buf, ZENDB_CRYPT_FILE_CHUNK_SIZE, NULL, 0, 0) != 0)
		{
			zendb_set_error(c, ZENDB_ERROR_INTERNAL, "Failed to encrypt a block");
			return 0;
		}
		if (fwrite(buf_out, 1, out_len, c->fd) != out_len)
		{
 ...
 */
int zendb_write_str(ZenDB *c, const char *x)
{
	uint16_t len;
 ...
 * @returns 1 on success, 0 on failure.
 */
int zendb_write_int64(ZenDB *c, uint64_t t)
{
#ifdef NATIVE_BIG_ENDIAN
	t = bswap_64(t);
#endif
	return zendb_write(c, &t, sizeof(t));
}
 ...
 * @returns 1 on success, 0 on failure.
 */
int zendb_write_int32(ZenDB *c, uint32_t t)
{
#ifdef NATIVE_BIG_ENDIAN
	t = bswap_32(t);
#endif
 ...
 * @returns 1 on success, 0 on failure.
 */
int zendb_write_int16(ZenDB *c, uint16_t t)
{
#ifdef NATIVE_BIG_ENDIAN
	t = bswap_16(t);
#endif
 ...
 * @returns 1 on success, 0 on failure.
 */
int zendb_write_char(ZenDB *c, char t)
{
	return zendb_write(c, &t, sizeof(t));
}
 ...
 * @note This is the internal function, api users must use one of the
 *       following functions instead:
 *       zendb_read_int64(), zendb_read_int32(), zendb_read_int16(),
 *       zendb_read_char(), zendb_read_str().
 */
static int zendb_read(ZenDB *c, void *rbuf, int len)
{
	char buf_in[ZENDB_CRYPT_FILE_CHUNK_SIZE + crypto_secretstream_xchacha20poly1305_ABYTES];
	unsigned long long out_len;
	unsigned char tag;
	size_t rlen;
	char *buf = rbuf;
	...
	if (c->mode != ZENDB_MODE_READ)
	{
		zendb_set_error(c, ZENDB_ERROR_API, "Read operation requested on a file opened for writing");
		return 0;
	}
	...
	do
	{
		...
		if (rlen == 0)
		{
			zendb_set_error(c, ZENDB_ERROR_IO, "Short read - premature end of file??");
			return 0;
		}
		if (crypto_secretstream_xchacha20poly1305_pull(&c->st, c->buf, &out_len, &tag, buf_in, rlen, NULL, 0) != 0)
		{
			zendb_set_error(c, ZENDB_ERROR_IO, "Failed to decrypt a block - either corrupt or wrong key");
			return 0;
		}

		/* This should be impossible as this is guaranteed not to happen by libsodium */
		if (out_len > ZENDB_CRYPT_FILE_CHUNK_SIZE)
			abort();
 ...
 * @returns 1 on success, 0 on failure.
 */
int zendb_read_int64(ZenDB *c, uint64_t *t)
{
	if (!zendb_read(c, t, sizeof(uint64_t)))
		return 0;
#ifdef NATIVE_BIG_ENDIAN
	*t = bswap_64(*t);
#endif
 ...
 * @returns 1 on success, 0 on failure.
 */
int zendb_read_int32(ZenDB *c, uint32_t *t)
{
	if (!zendb_read(c, t, sizeof(uint32_t)))
		return 0;
#ifdef NATIVE_BIG_ENDIAN
	*t = bswap_32(*t);
#endif
 ...
 * @returns 1 on success, 0 on failure.
 */
int zendb_read_int16(ZenDB *c, uint16_t *t)
{
	if (!zendb_read(c, t, sizeof(uint16_t)))
		return 0;
#ifdef NATIVE_BIG_ENDIAN
	*t = bswap_16(*t);
#endif
 ...
 * @returns 1 on success, 0 on failure.
 */
int zendb_read_str(ZenDB *c, char **x)
{
	uint16_t len;
	size_t size;
 ...
 * @returns 1 on success, 0 on failure.
 */
int zendb_read_char(ZenDB *c, char *t)
{
	if (!zendb_read(c, t, sizeof(char)))
		return 0;
	return 1;
}
...
void zendb_test_simple(void)
{
	...
	c = zendb_open("/tmp/test.db", ZENDB_MODE_WRITE, key);
	if (!c)
		fatal_error("Could not open test db for writing: %s", strerror(errno));

	if (!zendb_write_str(c, "Hello world!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"))
		fatal_error("Error on write 1");
	if (!zendb_write_str(c, "This is a test!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"))
	...
	c = zendb_open("/tmp/test.db", ZENDB_MODE_READ, key);
	if (!c)
		fatal_error("Could not open test db for reading: %s", strerror(errno));
	if (!zendb_read_str(c, &str))
 ...
 #define ZENDB_SPEED_TEST_BYTES 100000000
void zendb_test_speed(char *key)
{
	ZenDB *c;
	int i, len;
	char *str;
...
void zendb_test(void)
{
	//zendb_test_simple();
	fprintf(stderr, "**** TESTING ENCRYPTED ****\n");
	zendb_test_speed("test");
	fprintf(stderr, "**** TESTING UNENCRYPTED ****\n");
	zendb_test_speed(NULL);
}
#endif

/** TODO: document and implement
 */
const char *zendb_test_secret(const char *name)
{
	// FIXME: check if exists, if not then return an error, with a nice FAQ reference etc.
	return NULL; /* no error */
}

ZenDBConfig *zendb_copy_config(ZenDBConfig *src)
{
	ZenDBConfig *dst = safe_alloc(sizeof(ZenDBConfig));
...
ZenDBConfig *zendb_get_config(ZenDB *db)
{
	return zendb_copy_config(db->config);
}

void zendb_free_config(ZenDBConfig *c)
{
	if (!c)
		return;
	safe_free(c->salt);
...
static int zendb_config_identical(ZenDBConfig *one, ZenDBConfig *two)
{
...
static SecretCache *find_secret_cache(Secret *secr, ZenDBConfig *cfg)
{
	...
	{
		if (zendb_config_identical(c->config, cfg))
		{
			c->cache_hit = TStime();
			return c;
		}
...
static void zendb_add_to_secret_cache(Secret *secr, ZenDBConfig *cfg)
{
	...
	c->config = zendb_copy_config(cfg);
	c->cache_hit = TStime();
	AddListItem(c, secr->cache);
}

#ifdef DEBUGMODE
 #define ZENDB_EXPIRE_SECRET_CACHE_AFTER 1200
#else
 #define ZENDB_EXPIRE_SECRET_CACHE_AFTER 86400
#endif

/** Expire cached secret entries (previous Argon2 runs) */
EVENT(zendb_expire_secret_cache)
{
	Secret *s;
	...
	for (s = secrets; s; s = s->next)
	{
		...
		{
			c_next = c->next;
			if (c->cache_hit < TStime() - ZENDB_EXPIRE_SECRET_CACHE_AFTER)
			{
