/*
 *   Zen Internet Relay Chat Daemon, src/url.c
 *   (C) 2021 Bram Matthys and the ZenIRCd team
 *
...
void zen_https_initiate_connect(Download *handle);
int url_parse(const char *url, char **host, int *port, char **username, char **password, char **document);
void zen_https_connect_handshake(int fd, int revents, void *data);
...
void url_start_async(OutgoingWebRequest *request)
{
	...
	{
		https_cancel(handle, "Only https:// is supported (either rebuild ZenIRCd with curl support or use https)");
		return;
	}
	...
	if (request->store_in_file)
	{
		...
		filename = zen_getfilename(file);
		tmp = zen_mktemp(TMPDIR, filename ? filename : "download.conf");
	...
	if (is_valid_ip(handle->hostname))
	{
		...
		zen_https_initiate_connect(handle);
	} else
	{
...
void zen_https_initiate_connect(Download *handle)
{
	...
	fd_setselect(handle->fd, FD_SELECT_WRITE, zen_https_connect_handshake, handle);
}

// based on zen_tls_client_handshake()
void zen_https_connect_handshake(int fd, int revents, void *data)
{
...
// Based on zen_tls_connect_retry
 ...
 * Based on zen_tls_connect() but different return values.
 * @retval 1 connected
...
long long https_handle_response_body_memory(Download *handle, const char *ptr, long long write_sz)
{
	...
	if (size_required >= handle->memory_data_allocated - 1) // the -1 is for zero termination, even though it is binary..
	{
		...
		if (!newptr)
		{
			zen_log(ULOG_ERROR, "url", "URL_DOWNLOAD_MEMORY", NULL, "Async URL callback failed when reading returned data: out of memory?");
			safe_free(handle->memory_data);
...
void https_done(Download *handle)
{
	...
	else
	{
		if ((handle->last_modified > 0) && handle->filename)
			zen_setfilemodtime(handle->filename, handle->last_modified);
