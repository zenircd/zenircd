 ...
 * (C) Copyright 2004-2020 Bram Matthys (Syzop) and the ZenIRCd team
...
#if ZEN_VERSION_TIME >= 201915
	/* On ZenIRCd 4.2.3+ we can see the boot time (start time)
...
RPC_CALL_FUNC(rpc_connthrottle_status)
{
	...
	if (ucounter->disabled)
		json_object_set_new(result, "state", json_string_zen("disabled_by_oper"));
	else if (start_delay_remaining > 0)
		json_object_set_new(result, "state", json_string_zen("start_delay"));
	else if (reputation_gathering)
		json_object_set_new(result, "state", json_string_zen("reputation_gathering"));
	else if (ucounter->throttling_this_minute)
		json_object_set_new(result, "state", json_string_zen("throttling"));
	else
		json_object_set_new(result, "state", json_string_zen("monitoring"));
