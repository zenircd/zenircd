/*
 *   Zen Internet Relay Chat Daemon, src/conf.c
 *   (C) 1998-2000 Chris Behrens & Fred Jacobs (comstud, moogle)
 *   (C) 2000-2002 Carsten V. Munk and the ZenIRCd Team
 *
...
long config_checkval(const char *orig, unsigned short flags)
{
	...
	if (flags == CFG_YESNO)
	{
		for (text = value; *text; text++)
		{
			if (!isalnum(*text))
				continue;
			if (tolower(*text) == 'y' || (tolower(*text) == 'o' && tolower(*(text + 1)) == 'n') || *text == '1' || tolower(*text) == 't')
			{
				ret = 1;
				break;
			}
...
int zen_add_quotes_r(const char *i, char *o, size_t len)
{
	if (len == 0)
		return 0;
...
const char *zen_add_quotes(const char *str)
{
	static char qbuf[2048];

	*qbuf = '\0';
	zen_add_quotes_r(str, qbuf, sizeof(qbuf));
	return qbuf;
}
		...
		switch (*ptr)
		{
			...
			processchar:
				if ((*ptr == '*') && (*(ptr + 1) == '/'))
				{
					...
					config_status("WARNING: Starting with ZenIRCd 4.2.1 a /*-style comment stops as soon as the first */ is encountered. "
...
void config_error(FORMAT_STRING(const char *format), ...)
{
	...
	if ((ptr = strchr(buffer, '\n')) != NULL)
		*ptr = '\0';
	zen_log_raw(ULOG_ERROR, "config", "CONFIG_ERROR_GENERIC", NULL, buffer);
}
...
void config_status(FORMAT_STRING(const char *format), ...)
{
	...
	if ((ptr = strchr(buffer, '\n')) != NULL)
		*ptr = '\0';
	zen_log_raw(ULOG_INFO, "config", "CONFIG_INFO_GENERIC", NULL, buffer);
}
...
void config_warn(FORMAT_STRING(const char *format), ...)
{
	...
	if ((ptr = strchr(buffer, '\n')) != NULL)
		*ptr = '\0';
	zen_log_raw(ULOG_WARNING, "config", "CONFIG_WARNING_GENERIC", NULL, buffer);
}
...
void config_setdefaultsettings(Configuration *i)
{
	...
	/* - everyone */
	i->throttle_count = 3;
	i->throttle_period = 60; /* throttle protection: max 3 per 60s */
	...
	safe_strdup(i->tls_options->ciphers, ZENIRCD_DEFAULT_CIPHERS);
	safe_strdup(i->tls_options->ciphersuites, ZENIRCD_DEFAULT_CIPHERSUITES);
	...
	zen_add_mask_string(&i->central_spamfilter_except->mask, "*.irccloud.com");
...
{
	if (conf)
		zen_log(ULOG_ERROR, "config", "CONFIG_NOT_LOADED", NULL, "IRCd configuration failed to load");
...
int config_test(void)
{
	...
	} else if (!loop.booted)
	{
		...
		if (!init_tls())
		{
			config_error("Failed to load TLS (see errors above). ZenIRCd can not start.");
			config_load_failed();
			return -1;
		}
	}
	...
	postconf();
	zen_log(ULOG_INFO, "config", "CONFIG_LOADED", NULL, "Configuration loaded");
...
int config_read_file(const char *filename, const char *display_name)
{
	...
	} else
	{
		...
		{
			if (file_exists("zenircd.conf"))
			{
				config_error("Note that 'zenircd.conf' now belongs in the 'conf' subdirectory! (So move it to there)");
			} else
			{
				config_error("New to ZenIRCd? Be sure to read https://www.unrealircd.org/docs/Installing_%%28Windows%%29");
			}
		}
#endif
		return -1;
...
int config_test_blocks()
{
	...
	{
		config_error("It seems your set::snomask-on-oper and/or oper::snomask needs to be updated. Are you perhaps upgrading from an older version to ZenIRCd 6?");
...
void new_permissions_system(ConfigFile *conf, ConfigEntry *ce)
{
	...
	config_error("%s:%i: ZenIRCd 4.2.1 and higher have a new operclass permissions system.",
	             ce->file->filename, ce->line_number);
...
int _test_operclass(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
		...
		} else if (!strcmp(cep->name, "privileges"))
		{
...
int _test_oper(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (has_auto_login && has_broad_match)
	{
		...
		errors++;
	} else if (!has_password && has_broad_match)
	{
	...
	} else
	{
		if (!ce->value)
		{
			config_error("%s:%i: proxy { } blocks need to have a name, like proxy myproxy { }",
			             ce->file->filename, ce->line_number);
			errors++;
		} else if (!security_group_valid_name(ce->value))
		{
...
int _test_allow(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (has_ip && has_hostname)
	{
		...
		errors++;
	} else if (hostname_possible_silliness)
	{
		...
		if (!strcmp(cep->name, "incoming"))
		{
			...
			for (cepp = cep->items; cepp; cepp = cepp->next)
			{
				if (!strcmp(cepp->name, "match"))
				{
					...
					} else if (config_is_blankorempty(cepp, "link::incoming"))
					{
						errors++;
						continue;
					}
				} else if (!strcmp(cepp->name, "mask"))
				{
					if (cepp->value || cepp->items)
					{
						has_incoming_mask = 1;
...
int _test_require(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
		...
		} else if (config_is_blankorempty(cep, "require"))
		{
			errors++;
			continue;
		} else if (!strcmp(cep->name, "reason"))
...
#define CheckDuplicate(cep, name, display) \
	if (settings.has_##name) \
	{ \
		config_warn_duplicate((cep)->file->filename, cep->line_number, "set::" display); \
		continue; \
	} else \
		settings.has_##name = 1
...
void test_tlsblock(ConfigFile *conf, ConfigEntry *cep, int *totalerrors)
{
	...
	for (cepp = cep->items; cepp; cepp = cepp->next)
	{
		...
		} else if (!strcmp(cepp->name, "dh"))
		{
			 ...
			 */
			config_warn("%s:%d: Not reading DH file '%s'. ZenIRCd does not support old DH(E), we use modern ECDHE/EECDH. "
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
				...
				for (ceppp = cepp->items; ceppp; ceppp = ceppp->next)
				{
					...
					} else if (!strcmp(ceppp->name, "max-concurrent-conversations"))
					{
						...
						for (cep4 = ceppp->items; cep4; cep4 = cep4->next)
						{
							...
							} else if (!strcmp(cep4->name, "new-user-every"))
							{
		...
		} else if (!strcmp(cep->name, "dns"))
		{
			for (cepp = cep->items; cepp; cepp = cepp->next)
			{
				...
				} else if (!strcmp(cepp->name, "dnsbl"))
				{
					for (ceppp = cepp->items; ceppp; ceppp = ceppp->next)
					{
						if (!strcmp(ceppp->name, "timeout"))
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
		if (!strcmp(cep->name, "kline-address"))
		{
			...
			if (!strchr(cep->value, '@') && !strchr(cep->value, ':'))
			{
				...
				errors++;
				continue;
			} else if (match_simple("*@zenircd.com", cep->value) ||
			           match_simple("*@unrealircd.org", cep->value) ||
			           match_simple("unreal-*@lists.sourceforge.net", cep->value))
			{
				config_error("%s:%i: set::kline-address may not be an ZenIRCd Team address",
				             cep->file->filename, cep->line_number);
				errors++;
				continue;
			}
		} else if (!strcmp(cep->name, "gline-address"))
		{
			...
			if (!strchr(cep->value, '@') && !strchr(cep->value, ':'))
			{
				...
				continue;
			           ...
			           match_simple("unreal-*@lists.sourceforge.net", cep->value))
			{
				config_error("%s:%i: set::gline-address may not be an ZenIRCd Team address",
				             cep->file->filename, cep->line_number);
				errors++;
				continue;
			}
		...
		} else if (!strcmp(cep->name, "max-targets-per-command"))
		{
			for (cepp = cep->items; cepp; cepp = cepp->next)
			{
				...
				} else if (!strcasecmp(cepp->name, "USERHOST") ||
		...
		{
			config_error("%s:%d: set::restrict-extendedbans has no effect since ZenIRCd 6.0.0. "
		...
		} else if (!strcmp(cep->name, "anti-flood"))
		{
			...
			} else if (anti_flood_old_and_default == 1)
			{
				config_warn("%s:%d: the set::anti-flood block has been reorganized to be more flexible.",
				            cep->file->filename, cep->line_number);
				config_warn("To fix this warning, delete the anti-flood block from your configuration file "
				            "(file %s around line %d), this will make ZenIRCd use the built-in defaults.",
				            cep->file->filename, cep->line_number);
		...
		{
			config_warn("%s:%i: Timesync support has been removed from ZenIRCd. "
				...
				if (!strcmp(cepp->name, "except"))
				{
					if (cepp->value)
					{
						...
						config_warn("%s: %d: set::spamfilter::except is now a mask item. "
						            ...
						            "because future ZenIRCd versions will make this an error. "
				...
				} else if (!strcmp(cepp->name, "limit-ban-action"))
				{
					...
					if (n == 0)
					{
						config_error("%s:%i: set::central-spamfilter::limit-ban-action: unknown ban action '%s'",
						             cepp->file->filename, cepp->line_number, cepp->value);
						errors++;
					} else if ((n == BAN_ACT_SET) || (n == BAN_ACT_REPORT))
					{
		...
		} else if (!strcmp(cep->name, "best-practices"))
		{
			for (cepp = cep->items; cepp; cepp = cepp->next)
			{
				...
				} else if (!strcmp(cepp->name, "trusted-cert"))
				{
					bestpractices.trusted_cert = config_checkval(cepp->value, CFG_YESNO);
				} else if (!strcmp(cepp->name, "trusted-cert-valid-hostname"))
				{
					bestpractices.trusted_cert_valid_hostname = config_checkval(cepp->value, CFG_YESNO);
				} else if (!strcmp(cepp->name, "listen-nontls-port") || !strcmp(cepp->name, "listen-tls-only"))
				{
...
int _conf_alias(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
		if (!strcmp(cep->name, "format"))
		{
			...
			format->expr = zen_create_match(MATCH_PCRE_REGEX, cep->value, NULL);
			if (!format->expr)
...
int _test_alias(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
		...
		if (!strcmp(cep->name, "format"))
		{
			...
			has_format = 1;
			expr = zen_create_match(MATCH_PCRE_REGEX, cep->value, &err);
			if (!expr)
			{
...
int _test_secret(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
		...
		} else if (!strcmp(cep->name, "password-file"))
		{
			char *str;
			has_password_file = 1;
			CheckNull(cep);
		...
		} else if (!strcmp(cep->name, "password-prompt"))
		{
#ifdef _WIN32
		...
		} else if (!strcmp(cep->name, "password-url"))
		{
			config_error("%s:%d: secret::password-url is not supported yet in this ZenIRCd version.",
			             cep->file->filename, cep->line_number);
			errors++;
		} else
		{
...
void resource_download_complete(OutgoingWebRequest *request, OutgoingWebResponse *response)
{
	...
	} else
	{
		...
		{
			/* Copy to cache */
			const char *cache_file = zen_mkcache(request->url);
			zen_copyfileex(response->file, cache_file, 1);
			safe_strdup(rs->file, cache_file);
		}
	}
...
int rehash_internal(Client *client)
{
	...
	if (client == NULL)
		zen_log(ULOG_INFO, "config", "CONFIG_RELOAD", NULL, "Rehashing server configuration file [./zenircd rehash]");
...
int add_config_resource(const char *resource, int type, ConfigEntry *ce)
{
	...
	} else
	{
		...
		cache_file = zen_mkcache(rs->url);
		modtime = zen_getfilemodtime(cache_file);
		...
		if (modtime > 0)
		{
			...
			for (cep = ce->items; cep; cep = cep->next)
			{
				if (!strcmp(cep->name, "url-refresh"))
				{
					...
					/* Then remove the config item so it is not seen by the rest of zenircd.
...
int tls_tests(void)
{
	if (have_tls_listeners == 0)
	{
		...
		config_status("Add this to your zenircd.conf: listen { ip %s; port 6697; options { tls; }; };",
...
void link_generator(void)
{
	...
	printf("\n");
	printf("Add the following link block to the zenircd.conf on the OTHER side of the link\n");
	printf("(so NOT in the zenircd.conf on THIS machine). Here it is, just copy-paste:\n");
	printf("################################################################################\n");
	printf("link %s {\n"
	       "    incoming {\n"
	       "        mask *;\n"
...
DynamicSetOption *get_setting_for_user(Client *client, SetOption opt)
{
	...
	for (sg = securitygroups; sg; sg = sg->next)
	{
		...
		} else if (user_allowed_by_security_group(client, sg) &&
