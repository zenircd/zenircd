...
/* new FD management code, based on mowgli.eventloop from atheme, hammered into ZenIRCd by
 * me, nenolod.
 */
...
int fd_fileopen(const char *path, unsigned int flags)
{
	...
	snprintf(comment, sizeof comment, "File: %s", zen_getfilename(pathbuf));
