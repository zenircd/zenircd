/* server.* RPC calls
 * (C) Copyright 2022-.. Bram Matthys (Syzop) and the ZenIRCd team
...
RPC_CALL_FUNC(rpc_server_rehash)
{
	...
	if (!loop.rehashing)
	{
		zen_log(ULOG_INFO, "config", "CONFIG_RELOAD", client, "Rehashing server configuration file [by: $client.details]");
...
void json_expand_module(json_t *j, const char *key, Module *m, int detail)
{
	...
	json_object_set_new(child, "name", json_string_zen(m->header->name));
	json_object_set_new(child, "version", json_string_zen(m->header->version));
	json_object_set_new(child, "author", json_string_zen(m->header->author));
	json_object_set_new(child, "description", json_string_zen(m->header->description));
