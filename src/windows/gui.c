...
int AskCloseZen(HWND hWnd)
{
	if (MessageBox(hWnd, "Close ZenIRCd?", "Are you sure?", MB_YESNO | MB_ICONQUESTION) == IDNO)
		return 0;
	DoCloseZen(hWnd);
	exit(0);
}
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	 ...
	 * and we want to be in (f.e.) "C:\Program Files\ZenIRCd 6"
	 */
	chdir("..");
	...
	if ((hService = OpenService(hSCManager, "ZenIRCd", SC_MANAGER_CONNECT)))
	{
		int save_err = 0;
	...
	hWnd = CreateDialog(hInstance, "ZenIRCd", 0, (DLGPROC)MainDLG);
	hwIRCDWnd = hWnd;

	TaskBarCreated();

	if (InitZenIRCd(__argc, __argv) != 1)
	{
		MessageBox(NULL, "ZenIRCd has failed to initialize in InitZenIRCd()", "ZenIRCd Initalization Error", MB_OK);
		return FALSE;
	}
...
LRESULT CALLBACK MainDLG(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case WM_COMMAND:
		{
			...
			if (!loop.booted)
			{
				MessageBox(NULL, "ZenIRCd not booted due to configuration errors. "
				                 "Check other window for error details. Then close that window, "
				                 "fix the errors and start ZenIRCd again.",
				           "ZenIRCd not started",
				           MB_OK);
				return FALSE;
			}
...
{
	return FromVarDLG(hDlg, message, wParam, lParam, "ZenIRCd License", gnulicense);
}

LRESULT CALLBACK InfoDLG(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	return FromVarDLG(hDlg, message, wParam, lParam, "ZenIRCd Team", zeninfo);
}

LRESULT CALLBACK CreditsDLG(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	return FromVarDLG(hDlg, message, wParam, lParam, "ZenIRCd Credits", zencredits);
}
...
LRESULT CALLBACK FromFileReadDLG(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		case WM_INITDIALOG:
		{
			...
			StringCbPrintf(szText, sizeof(szText), "ZenIRCd Viewer - %s", (unsigned char *)lParam);
		...
		case WM_COMMAND:
			...
			if (HIWORD(wParam) == BN_DBLCLK)
			{
				if (LOWORD(wParam) == IDC_URL)
					...
					ShellExecute(NULL, "open", "mailto:zen-users@lists.sourceforge.net", NULL, NULL,
					             SW_MAXIMIZE);
				EndDialog(hDlg, TRUE);
				return 0;
			}
