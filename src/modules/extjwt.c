...
char *extjwt_make_payload(Client *client, Channel *channel, struct extjwt_config *config)
{
	...
	json_object_set_new(payload, "iss", json_string_zen(me.name));
	json_object_set_new(payload, "sub", json_string_zen(client->name));
	json_object_set_new(payload, "account", json_string_zen(IsLoggedIn(client) ? client->user->account : ""));

	if (config->vfy) /* also add the URL */
		json_object_set_new(payload, "vfy", json_string_zen(config->vfy));
	...
	if (channel)
	{
		...
		json_object_set_new(payload, "channel", json_string_zen(channel->name));
