 ...
 * (C) Copyright 2015-2019 Bram Matthys (Syzop) and the ZenIRCd team.
 ...
 * is data/reputation.db (often ~/zenircd/data/reputation.db).
 *
 ...
 * would delay the ZenIRCd boot process only for 0.1 second.
...
int reputation_config_test(ConfigFile *cf, ConfigEntry *ce, int type, int *errs)
{
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
		...
		{
			config_error("%s:%i: this feature is not implemented yet in this ZenIRCd version",
		...
		} else if (!cep->value)
		{
			...
			errors++;
		} else if (!strcmp(cep->name, "score-bump-timer-minimum-channel-members"))
		{
...
void reputation_load_db_old(void)
{
	...
	if (!parse_db_header_old(buf))
	{
		config_error("WARNING: Cannot load database %s. Error reading header. "
		             ...
		             "ZenIRCd version perhaps? This is not supported.",
		             cfg.database);
		fclose(fd);
		return;
	}
...
#define R_SAFE(x) \
	...
	{ \
		if (!(x)) \
		{ \
			config_warn("[reputation] Read error from database file '%s' (possible corruption): %s", cfg.database, zendb_get_error_string()); \
			zendb_close(db); \
			safe_free(ip); \
			return 0; \
		} \
...
int reputation_load_db_new(ZenDB *db)
{
	...
	R_SAFE(zendb_read_int64(db, &l_db_version)); /* reputation db version */
	if (l_db_version > 2)
	{
		config_error("[reputation] Reputation DB is of a newer version (%ld) than supported by us (%ld). "
		             "Did you perhaps downgrade your ZenIRCd?",
		             (long)l_db_version, (long)2);
		zendb_close(db);
		return 0;
	}
	R_SAFE(zendb_read_int64(db, &l_starttime)); /* starttime of data gathering */
	R_SAFE(zendb_read_int64(db, &l_writtentime)); /* current time */
	R_SAFE(zendb_read_int64(db, &count)); /* number of entries */
	...
	for (i = 0; i < count; i++)
	{
		R_SAFE(zendb_read_str(db, &ip));
		R_SAFE(zendb_read_int16(db, &score));
		R_SAFE(zendb_read_int64(db, &last_seen));
 ...
 * 2) Otherwise, open with zendb routine
 * 3) If that fails due to a password provided but the file is zendb without password
...
int reputation_load_db(void)
{
	...
	*buf = '\0';
	...
	/* Otherwise, it is an zendb, crypted or not */
	...
	if (!db)
	{
		...
		if (!db)
		{
			config_error("[reputation] Unable to open the database file '%s' for reading: %s", cfg.database, zendb_get_error_string());
			return 0;
		}
	}
...
int reputation_save_db(void)
{
...
#ifdef TEST
	zen_log(ULOG_DEBUG, "reputation", "REPUTATION_TEST", NULL, "Reputation in running in test mode. Saving DB's....");
#endif
	...
	/* Write header */
	W_SAFE(zendb_write_int64(db, 2)); /* reputation db version */
	W_SAFE(zendb_write_int64(db, reputation_starttime)); /* starttime of data gathering */
	W_SAFE(zendb_write_int64(db, TStime())); /* current time */
	...
	for (i = 0; i < REPUTATION_HASH_TABLE_SIZE; i++)
		for (e = ReputationHashTable[i]; e; e = e->next)
			count++;
	W_SAFE(zendb_write_int64(db, count)); /* Number of DB entries */
	...
	for (i = 0; i < REPUTATION_HASH_TABLE_SIZE; i++)
	{
		for (e = ReputationHashTable[i]; e; e = e->next)
		{
			W_SAFE(zendb_write_str(db, e->ip));
			W_SAFE(zendb_write_int16(db, e->score));
			W_SAFE(zendb_write_int64(db, e->last_seen));
		}
	}
...
#define MARKER_REGISTERED_USER   (marker + 1)
	...
	list_for_each_entry(client, &client_list, client_node)
	{
		...
		} else if ((e->marker == MARKER_UNREGISTERED_USER) && IsLoggedIn(client) && (e->score < REPUTATION_SCORE_CAP))
		{
 ...
 *
 * Since ZenIRCd 6.0.2+ there is now also asterisk-score-asterisk:
