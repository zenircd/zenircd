/*
 *   Zen Internet Relay Chat Daemon, src/socket.c
...
int zen_listen_inet(ConfigItem_listen *listener)
{
...
int zen_listen_unix(ConfigItem_listen *listener)
{
	...
	if (!getrlimit(RLIMIT_FD_MAX, &limit))
	{
		...
		if (m < 100)
		{
			fprintf(stderr, "\nERROR: Your OS has a limit placed on this account.\n"
			                "This machine only allows ZenIRCd to handle a maximum of %ld open connections/files, which is VERY LOW.\n"
...
void completed_connection(int fd, int revents, void *data)
{
	...
	{
		/* Due to delayed zen_tls_connect call */
...
void start_dns_and_ident_lookup(Client *client)
{
	...
	if (client->local->hostp)
	{
		zen_free_hostent(client->local->hostp);
...
int zen_connect(int fd, const char *ip, int port, SocketType socket_type)
{
	int n;
 ...
 */
int zen_bind(int fd, const char *ip, int port, SocketType socket_type)
{
...
void init_winsock(void)
{
	...
	{
		MessageBox(NULL, "Unable to initialize WinSock", "ZenIRCd Initalization Error", MB_OK);
