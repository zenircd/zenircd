/************************************************************************
 *   IRC - Internet Relay Chat, windows/zensvc.c
...
void show_usage()
{
	fprintf(stderr, "zensvc start|stop|rehash|restart|install|uninstall|config <option> <value>");
...
SC_HANDLE zen_open_service_manager(void)
{
...
int main(int argc, char *argv[])
{
	...
	if (!strcasecmp(argv[1], "install"))
	{
		...
		hSCManager = zen_open_service_manager();
		...
		strcpy(binpath, path);
		strcat(binpath, "\\ZenIRCd.exe");
		hService = CreateService(hSCManager, "ZenIRCd", "ZenIRCd",
		...
		if (hService)
		{
			...
			printf("ZenIRCd NT Service successfully installed\n");
			...
			printf("\n[!!!] IMPORTANT: By default the network service user cannot write to the \n"
			       "ZenIRCd 6 folder and this will make ZenIRCd fail to boot without\n"
			       "writing any meaningful error to the log files.\n"
			       "You have two options:\n"
			       "1) Manually grant FULL permissions to NT AUTHORITY\\NetworkService\n"
			       "   for the ZenIRCd 6 folder, all its subfolders and files.\n"
			       "OR, easier and recommended:\n"
			       "2) just re-run the ZenIRCd installer and select 'Install as a service',\n"
			       "   which sets all the necessary permissions automatically.\n");
		} else
		{
			printf("Failed to install ZenIRCd NT Service - %s", show_error(GetLastError()));
		}
		CloseServiceHandle(hSCManager);
		return 0;
	} else if (!strcasecmp(argv[1], "uninstall"))
	{
		SC_HANDLE hSCManager = zen_open_service_manager();
		SC_HANDLE hService = OpenService(hSCManager, "ZenIRCd", DELETE);
		if (DeleteService(hService))
			printf("ZenIRCd NT Service successfully uninstalled\n");
		else
			printf("Failed to uninstall ZenIRCd NT Service - %s\n", show_error(GetLastError()));
		CloseServiceHandle(hService);
		CloseServiceHandle(hSCManager);
		return 0;
	...
	{
		SC_HANDLE hSCManager = zen_open_service_manager();
		SC_HANDLE hService = OpenService(hSCManager, "ZenIRCd", SERVICE_START);
		if (StartService(hService, 0, NULL))
			printf("ZenIRCd NT Service successfully started");
		else
			printf("Failed to start ZenIRCd NT Service - %s", show_error(GetLastError()));
		CloseServiceHandle(hService);
		CloseServiceHandle(hSCManager);
		return 0;
	} else if (!strcasecmp(argv[1], "stop"))
	{
		...
		SC_HANDLE hSCManager = zen_open_service_manager();
		SC_HANDLE hService = OpenService(hSCManager, "ZenIRCd", SERVICE_STOP);
		ControlService(hService, SERVICE_CONTROL_STOP, &status);
		printf("ZenIRCd NT Service successfully stopped");
		CloseServiceHandle(hService);
		CloseServiceHandle(hSCManager);
		return 0;
	} else if (!strcasecmp(argv[1], "restart"))
	{
		SERVICE_STATUS status;
		SC_HANDLE hSCManager = zen_open_service_manager();
		SC_HANDLE hService = OpenService(hSCManager, "ZenIRCd", SERVICE_STOP | SERVICE_START);
		ControlService(hService, SERVICE_CONTROL_STOP, &status);
		if (StartService(hService, 0, NULL))
			printf("ZenIRCd NT Service successfully restarted");
		CloseServiceHandle(hService);
		CloseServiceHandle(hSCManager);
		return 0;
	} else if (!strcasecmp(argv[1], "rehash"))
	{
		SERVICE_STATUS status;
		SC_HANDLE hSCManager = zen_open_service_manager();
		SC_HANDLE hService = OpenService(hSCManager, "ZenIRCd", SERVICE_USER_DEFINED_CONTROL);
		ControlService(hService, IRCD_SERVICE_CONTROL_REHASH, &status);
		printf("ZenIRCd NT Service successfully rehashed");
	} else if (!strcasecmp(argv[1], "config"))
	{
		SERVICE_STATUS status;
		SC_HANDLE hSCManager = zen_open_service_manager();
		SC_HANDLE hService = OpenService(hSCManager, "ZenIRCd", SERVICE_CHANGE_CONFIG | SERVICE_START);
		if (argc < 3)
		{
			show_usage();
			return -1;
		...
		if (!strcasecmp(argv[2], "startup"))
		{
			if (ChangeServiceConfig(hService, SERVICE_NO_CHANGE,
				...
				printf("ZenIRCd NT Service configuration changed");
			else
				printf("ZenIRCd NT Service configuration change failed - %s", show_error(GetLastError()));
