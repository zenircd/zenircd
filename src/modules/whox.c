 ...
 * we had in ZenIRCd in 3.2.x and 4.0.x and that the WHOX spec defines
 ...
 * try to warn our users in case they use the 'original' ZenIRCd
...
static int convert_classical_who_request(Client *client, int *parc, const char *parv[], const char **orig_mask, struct who_format *fmt)
{
	...
	/* Figure out if the user is doing a 'classical' ZenIRCd request,
	    ...
	    (*parc < 4))
	{
		...
		} else
		{
			 ...
			 * ZenIRCd-style WHO request which has
