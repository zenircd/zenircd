/*
 * RPC module - for remote management of ZenIRCd
 * (C)Copyright 2022 Bram Matthys and the ZenIRCd team
...
int rpc_handle_webrequest(Client *client, WebRequest *web)
{
	...
	if (!strcmp(web->uri, "/api"))
	{
		...
		{
			webserver_send_response(client, 200, "To use the ZenIRCd RPC API you need to make a POST request. See https://www.unrealircd.org/docs/RPC\n");
			return 0;
		}
...
void _rpc_error(Client *client, json_t *request, JsonRpcError error_code, const char *error_message)
{
	...
	json_object_set_new(j, "jsonrpc", json_string_zen("2.0"));
	if (method)
		json_object_set_new(j, "method", json_string_zen(method));
	if (id)
...
int parse_rpc_call(Client *client, json_t *mainrequest, json_t *request, const char **method, RPCHandler **handler)
{
	...
	*handler = NULL;
	...
	if ((str = json_string_value(id)))
	{
		if (strlen(str) > 32)
		{
			rpc_error(client, mainrequest, JSON_RPC_ERROR_INVALID_REQUEST, "The 'id' cannot be longer than 32 characters in ZenIRCd JSON-RPC");
			return 0;
		}
		if (strchr(str, '\n') || strchr(str, '\r'))
		{
			rpc_error(client, mainrequest, JSON_RPC_ERROR_INVALID_REQUEST, "The 'id' may not contain \n or \r in ZenIRCd JSON-RPC");
			return 0;
		}
	} else if (!json_is_integer(id))
	{
		rpc_error(client, mainrequest, JSON_RPC_ERROR_INVALID_REQUEST, "The 'id' must be a string or an integer in ZenIRCd JSON-RPC");
		return 0;
	}
...
RPC_CALL_FUNC(rpc_rpc_info)
{
	...
	{
		item = json_object();
		json_object_set_new(item, "name", json_string_zen(r->method));
		if (r->owner)
		{
			json_object_set_new(item, "module", json_string_zen(r->owner->header->name));
			json_object_set_new(item, "version", json_string_zen(r->owner->header->version));
		}
...
int rpc_handle_server_quit(Client *client, MessageTag *mtags)
{
	...
	for (or = outstanding_rrpc_list; or; or = or_next)
	{
		...
		if (!strcmp(client->id, or->destination))
		{
			...
			{
				json_t *j = json_object();
				json_object_set_new(j, "id", json_string_zen(or->requestid));
	...
	for (or = outstanding_rrpc_list; or; or = or_next)
	{
		...
		if (or->sent < deadline)
		{
			...
			if (client)
			{
				...
				json_object_set_new(request, "id", json_string_zen(or->requestid));
...
void _rpc_send_request_to_remote(Client *source, Client *target, json_t *request)
{
	...
	if (!requestid)
	{
		...
		rpc_error(source, NULL, JSON_RPC_ERROR_INVALID_REQUEST, "The 'id' must be a string or an integer in ZenIRCd JSON-RPC");
		return;
	}
...
int rpc_json_expand_client_server(Client *client, int detail, json_t *j, json_t *child)
{
	...
	for (; nv; nv = nv->next)
	{
		...
		json_object_set_new(e, "name", json_string_zen(nv->name));
		json_object_set_new(e, "version", json_string_zen(nv->value));
