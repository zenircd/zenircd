...
LRESULT CALLBACK FromFileDLG(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (message == WM_FINDMSGSTRING)
	{
		...
		if (fr->Flags & FR_FINDNEXT)
		{
			...
			if (SendMessage(hRich, EM_FINDTEXTEX, flags, (LPARAM)&ft) == -1)
				MessageBox(NULL, "ZenIRCd has finished searching the document",
	...
	switch (message)
	{
		case WM_INITDIALOG:
		{
			...
			if (file)
				wsprintf(szText, "ZenIRCd Editor - %s", file);
			else
				strcpy(szText, "ZenIRCd Editor - New File");
		...
		case WM_COMMAND:
			...
			if (LOWORD(wParam) == IDM_NEW)
			{
				...
				if (SendMessage(GetDlgItem(hDlg, IDC_TEXT), EM_GETMODIFY, 0, 0) != 0)
				{
					...
					ans = MessageBox(hDlg, text, "ZenIRCd", MB_YESNOCANCEL | MB_ICONWARNING);
					if (ans == IDNO)
						newfile = TRUE;
					if (ans == IDCANCEL)
						return TRUE;
