 ...
 * (C) Copyright 2015-.. Syzop and the ZenIRCd team
 *
...
void zen_delete_masks(ConfigItem_mask *m)
{
...
void zen_add_mask_string(ConfigItem_mask **head, const char *name)
{
...
static void zen_add_mask(ConfigItem_mask **head, ConfigEntry *ce)
{
	/* Since we allow both mask "xyz"; and mask { abc; def; };... */
	if (ce->value)
		zen_add_mask_string(head, ce->value);
	else
		zen_add_mask_string(head, ce->name);
}

/** Add mask entries from config */
void zen_add_masks(ConfigItem_mask **head, ConfigEntry *ce)
{
	if (ce->items)
	{
		ConfigEntry *cep;
		for (cep = ce->items; cep; cep = cep->next)
			zen_add_mask(head, cep);
	} else
	{
		zen_add_mask(head, ce);
	}
}

ConfigItem_mask *zen_duplicate_masks(ConfigItem_mask *existing)
{
 ...
 */
int zen_mask_match(Client *client, ConfigItem_mask *mask)
{
	int retval = 1;
	ConfigItem_mask *m;
 ...
 * @returns 1 on match, 0 on non-match.
 */
int zen_mask_match_string(const char *name, ConfigItem_mask *mask)
{
	int retval = 1;
	ConfigItem_mask *m;
	...
	} else
	{
		if (!strcasecmp(ce->value, "unknown-users"))
		{
			config_error("%s:%i: The 'unknown-users' group is a special group that is the "
			             ...
			             "config file, as it is created automatically by ZenIRCd.",
			             ce->file->filename, ce->line_number);
			errors++;
			return errors;
		}
...
int conf_match_item(ConfigFile *conf, ConfigEntry *cep, SecurityGroup **block)
{
	...
	{
		zen_add_masks(&s->mask, cep);
	} else if (!strcmp(cep->name, "ip"))
	{
		zen_add_names(&s->ip, cep);
	} else if (!strcmp(cep->name, "server-port"))
	{
		zen_add_names(&s->server_port, cep);
	} else if (!strcmp(cep->name, "security-group"))
	{
		zen_add_names(&s->security_group, cep);
	...
	{
		zen_add_names(&s->destination, cep);
	...
	{
		zen_add_masks(&s->exclude_mask, cep);
	} else if (!strcmp(cep->name, "exclude-ip"))
	{
		zen_add_names(&s->exclude_ip, cep);
	} else if (!strcmp(cep->name, "exclude-server-port"))
	{
		zen_add_names(&s->exclude_server_port, cep);
	} else if (!strcmp(cep->name, "exclude-security-group"))
	{
		zen_add_names(&s->exclude_security_group, cep);
	} else if (!strcmp(cep->name, "exclude-rule"))
	{
	...
	{
		zen_add_names(&s->exclude_destination, cep);
	} else
	{
		...
		if (!strncmp(cep->name, "exclude-", 8))
		{
			...
			if (findmod_by_bantype_raw(name, strlen(name)))
				zen_add_name_values(&s->exclude_extended, name, cep);
			else
				return 0; /* Unhandled */
		} else
		{
			/* Extended (inclusive) */
			if (findmod_by_bantype_raw(name, strlen(name)))
				zen_add_name_values(&s->extended, name, cep);
			else
				return 0; /* Unhandled */
		}
	}
...
void free_security_group(SecurityGroup *s)
{
	...
	zen_delete_masks(s->mask);
	zen_delete_masks(s->exclude_mask);
...
SecurityGroup *duplicate_security_group(SecurityGroup *s)
{
	...
	n->mask = zen_duplicate_masks(s->mask);
	n->exclude_mask = zen_duplicate_masks(s->exclude_mask);
...
int user_allowed_by_security_group_context(Client *client, SecurityGroup *s, crule_context *context)
{
	...
	if (s->exclude_tls && (IsSecureConnect(client) || (MyConnect(client) && IsSecure(client))))
		goto user_not_allowed;
	if (s->exclude_mask && zen_mask_match(client, s->exclude_mask))
		goto user_not_allowed;
	if (s->exclude_ip && zen_match_iplist(client, s->exclude_ip))
		goto user_not_allowed;
	...
	if (s->tls && (IsSecureConnect(client) || (MyConnect(client) && !IsUser(client) && IsSecure(client))))
		goto user_allowed;
	if (s->mask && zen_mask_match(client, s->mask))
		goto user_allowed;
	if (s->ip && zen_match_iplist(client, s->ip))
		goto user_allowed;
