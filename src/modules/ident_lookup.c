...
static int ident_lookup_connect(Client *client)
{
	...
	{
		zen_log(ULOG_FATAL, "io", "IDENT_ERROR_MAXCLIENTS", client,
	...
	zen_bind(client->local->authfd, client->local->listener->ip, 0, client->local->socket_type);

	/* And connect... */
	if (!zen_connect(client->local->authfd, client->ip, 113, client->local->socket_type))
	{
		ident_lookup_failed(client);
		return 0;
	}
