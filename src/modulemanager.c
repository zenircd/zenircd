/* ZenIRCd module manager.
 * (C) Copyright 2019 Bram Matthys ("Syzop") and the ZenIRCd Team.
...
struct ManagedModule {
	...
	char *sha256sum;
	time_t mtime;
	char *min_zenircd_version;
	char *max_zenircd_version;
	char *description;
	char *compile_flags;
...
int mm_module_file_config(ManagedModule *m, ConfigEntry *ce)
{
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
		...
		} else if (!strcmp(cep->name, "min-zenircd-version"))
		{
			CheckNull(cep);
			safe_strdup(m->min_zenircd_version, cep->value);
		} else if (!strcmp(cep->name, "max-zenircd-version"))
		{
			CheckNull(cep);
			safe_strdup(m->max_zenircd_version, cep->value);
	...
	if (!m->min_zenircd_version)
	{
		config_error("%s:%d: module::min-zenircd-version missing", m->name, ce->line_number);
		return 0;
	}

	/* max_zenircd_version is optional */
...
char *mm_sourceslist_file(void)
{
	...
	if (!file_exists(buf1))
	{
		/* Possibly ZenIRCd is not installed yet, so use this one */
...
void free_managed_module(ManagedModule *m)
{
	...
	safe_free(m->min_zenircd_version);
	safe_free(m->max_zenircd_version);
...
ManagedModule *mm_repo_module_config(char *repo_url, ConfigEntry *ce)
{
	...
	if (!m->min_zenircd_version)
	{
		config_error("%s:%d: module::min-zenircd-version missing", repo_url, ce->line_number);
		goto fail_mm_repo_module_config;
	}
	/* max_zenircd_version is optional */
	if (!m->description)
...
int mm_refresh_repository(void)
{
	...
	if (!file_exists(TMPDIR))
	{
		...
		if (!file_exists(TMPDIR))
		{
			 ...
			 * eg if ~/zenircd does not exist at all then ~/zenircd/tmp
	...
	if (numrepos == 0)
	{
		fprintf(stderr, "ERROR: No repositories listed in module repository list. "
		                "Did you remove the default ZenIRCd repository?\n"
		                "All commands, except for './zenircd module uninstall third/name-of-module', are unavailable.\n");
		return 0;
	}
...
{
	if (strchr(m->min_zenircd_version, '*'))
	{
		/* By wildcard, eg: "5.*" */
		if (!match_simple(m->min_zenircd_version, VERSIONONLY))
			return 0;
	} else
	{
		/* By strcmp, eg: "5.0.0" */
		if (strnatcasecmp(m->min_zenircd_version, VERSIONONLY) > 0)
			return 0;
	}
	if (m->max_zenircd_version)
	{
		if (strchr(m->max_zenircd_version, '*'))
		{
			/* By wildcard, eg: "5.*" */
			if (!match_simple(m->max_zenircd_version, VERSIONONLY))
				return 0;
		} else
		{
			/* By strcmp, eg: "5.0.5" */
			if (strnatcasecmp(m->max_zenircd_version, VERSIONONLY) <= 0)
				return 0;
		}
	}
	return 1;
...
char *mm_get_module_status_string_long(ManagedModule *m)
{
	...
	if (status == 0)
		...
		return "Unavailable for your ZenIRCd version";
	else if (status == MMMS_INSTALLED)
		return "Installed and up to date";
	else if (status == (MMMS_INSTALLED | MMMS_UNAVAILABLE))
		return "Installed, an upgrade is available but not for your ZenIRCd version";
	else if (status == (MMMS_INSTALLED | MMMS_UPGRADE_AVAILABLE))
...
void mm_list(char *searchname)
{
	...
	printf("\nStatus column legend:\n"
	       ...
	       "inst/UNAV  : module installed, upgrade exists but is not available for your ZenIRCd version (too old ZenIRCd version?)\n"
	       "unav       : module not available for your ZenIRCd version\n"
	       "UNKNOWN    : module does not exist in any repository (perhaps you installed it manually?), module will be left untouched\n");

	printf("\nFor more information about a particular module, use './zenircd module info name-of-module'\n\n");
	print_documentation();
}
...
int mm_compile(ManagedModule *m, const char *tmpfile, int test, int upgrade)
{
	...
	basename = zen_getfilename(test ? tmpfile : m->name);
...
int mm_install(int argc, char *args[], int upgrade)
{
	...
	if (upgrade)
	{
		...
		{
			fprintf(stderr, "ERROR: Module '%s' exists, but is not compatible with your ZenIRCd version:\n"
			                "Your ZenIRCd version  : %s\n"
			                "Minimum version required : %s\n",
			        name,
			        VERSIONONLY,
			        m->min_zenircd_version);
			if (m->max_zenircd_version)
				fprintf(stderr, "Maximum version          : %s\n", m->max_zenircd_version);
			return 0;
		}
...
void mm_upgrade(int argc, char *args[])
{
		...
		for (i = 0; i < n_modules; i++)
		{
			...
			if (status == MMMS_UNAVAILABLE)
			{
				fprintf(stderr, "ERROR: Module '%s' exists, but is not compatible with your ZenIRCd version:\n"
				                "Your ZenIRCd version  : %s\n"
				                "Minimum version required : %s\n",
				        name, VERSIONONLY, m->min_zenircd_version);
				if (m->max_zenircd_version)
					fprintf(stderr, "Maximum version          : %s\n", m->max_zenircd_version);
				all_ok = 0;
				continue;
	...
	if (update_unavailable)
		printf("%d module(s) have updates but not for your ZenIRCd version\n", update_unavailable);
	if ((n = count_unknown_modules()))
		printf("%d module(s) are unknown/untracked\n", n);

	printf("For more details, you can always run ./zenircd module list\n");
}
...
void mm_info(int argc, char *args[])
{
	...
	if (!name)
	{
		fprintf(stderr, "ERROR: Use: zenircd module info name-of-module\n");
		exit(-1);
	}
...
void mm_usage(void)
{
	fprintf(stderr, "Use any of the following actions:\n"
	                "zenircd module list                              List all the available and installed modules\n"
	                "zenircd module info name-of-module               Show more information about the module\n"
	                "zenircd module install name-of-module [name2 ...] Install one or more modules\n"
	                "zenircd module uninstall name-of-module [name2 ...] Uninstall one or more modules\n"
	                "zenircd module upgrade name-of-module [name2 ...] Upgrade one or more modules (if needed)\n"
	                "zenircd module upgrade                           Upgrade all modules (if needed)\n"
	                "zenircd module generate-repository               Generate a repository index (you are\n"
...
void print_md_block(FILE *fdo, ManagedModule *m)
{
	...
	fprintf(fdo, "\tdescription \"%s\";\n", zen_add_quotes(m->description));
	fprintf(fdo, "\tversion \"%s\";\n", zen_add_quotes(m->version));
	fprintf(fdo, "\tauthor \"%s\";\n", zen_add_quotes(m->author));
	fprintf(fdo, "\tdocumentation \"%s\";\n", zen_add_quotes(m->documentation));
	fprintf(fdo, "\ttroubleshooting \"%s\";\n", zen_add_quotes(m->troubleshooting));
	fprintf(fdo, "\tsource \"%s\";\n", zen_add_quotes(m->source));
	fprintf(fdo, "\tsha256sum \"%s\";\n", zen_add_quotes(m->sha256sum));
	fprintf(fdo, "\tlast-updated \"%s\";\n", zen_add_quotes(timestamp_iso8601(m->mtime)));
	fprintf(fdo, "\tmin-zenircd-version \"%s\";\n", zen_add_quotes(m->min_zenircd_version));
	if (m->max_zenircd_version)
		fprintf(fdo, "\tmax-zenircd-version \"%s\";\n", zen_add_quotes(m->max_zenircd_version));
	if (m->compile_flags)
		fprintf(fdo, "\tcompile-flags \"%s\";\n", zen_add_quotes(m->compile_flags));
	if (m->post_install_text)
	{
		MultiLine *l;
...
{
	fprintf(stderr, "Usage: ./zenircd module generate-repository <url base path> <directory-with-modules> <name of output file> [optional-minimum-version-filter]\n");
	fprintf(stderr, "For example: ./zenircd module generate-repository https://www.unrealircd.org/modules/ src/modules/third modules.lst\n");
}
...
void mm_generate_repository(int argc, char *args[])
{
	...
	while ((dir = readdir(fd)))
	{
		...
		if (filename_has_suffix(fname, ".c"))
		{
			...
			m->mtime = zen_getfilemodtime(fullname);
			snprintf(m->source, 512, "%s%s.c", urlbasepath, modname + 6);
			/* filter */
			if (minversion && m->min_zenircd_version && strncmp(minversion, m->min_zenircd_version, strlen(minversion)))
				hide = 1;
			/* /filter */
			if (!hide)
...
void mm_parse_c_file(int argc, char *args[])
{
	...
	if (!fullname)
	{
		fprintf(stderr, "Usage: ./zenircd module parse-c-file path/to/file.c\n");
		exit(-1);
	}
	...
	basename = zen_getfilename(fullname);
...
void mm_self_test(void)
{
	...
	if (!file_exists(BUILDDIR))
	{
		fprintf(stderr, "ERROR: Directory %s does not exist.\n"
		                "The ZenIRCd source is required for the module manager to work!\n",
		        BUILDDIR);
		exit(-1);
	} else
	{
		...
		if (!file_exists(name))
		{
			fprintf(stderr, "ERROR: Directory %s exists, but your ZenIRCd is not compiled yet.\n"
			                "You must compile your ZenIRCd first (run './Config', then 'make install')\n",
			        BUILDDIR);
			exit(-1);
		}
	}
