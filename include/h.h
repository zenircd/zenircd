/************************************************************************
 *   Zen Internet Relay Chat Daemon, include/h.h
...
extern EVENT(zendns_removeoldrecords);
extern EVENT(zendb_expire_secret_cache);
extern void init_glines(void);
extern void tkl_init(void);
extern void process_clients(void);
extern void zendb_test(void);
...
/** Memory allocation and deallocation functions and macros that should be used in ZenIRCd.
 ...
 * need this, as it's only used in 1 place in ZenIRCd.
 */
...
extern int InitZenIRCd(int argc, char **);
...
extern char *zen_mktemp(const char *dir, const char *suffix);
extern char *zen_getpathname(const char *filepath, char *path);
extern const char *zen_getfilename(const char *path);
extern const char *zen_getmodfilename(const char *path);
extern int zen_create_directory_structure_for_file(const char *fname, mode_t mode);
extern int zen_create_directory_structure(const char *dname, mode_t mode);
extern int zen_mkdir(const char *pathname, mode_t mode);
extern int zen_copyfile(const char *src, const char *dest);
extern int zen_copyfileex(const char *src, const char *dest, int tryhardlink);
extern time_t zen_getfilemodtime(const char *filename);
extern void zen_setfilemodtime(const char *filename, time_t mtime);
extern int zen_touch(const char *filename, time_t mtime);
...
extern char *zen_encodespace(const char *s);
extern char *zen_decodespace(const char *s);
...
extern MODVAR void (*do_zen_log_remote_deliver)(LogLevel loglevel, const char *subsystem, const char *event_id, MultiLine *msg, const char *json_serialized);
...
extern MODVAR int (*zen_match_iplist)(Client *client, NameList *l);
...
extern MODVAR char *(*zen_expand_string)(const char *str, char *buf, size_t buflen, NameValuePrioList *nvp, int buildvarstring_options, Client *client);
...
extern int zen_tls_accept(Client *acptr, int fd);
extern int zen_tls_connect(Client *acptr, int fd);
extern int SSL_smart_shutdown(SSL *ssl);
extern const char *ssl_error_str(int err, int my_errno);
extern void zen_tls_client_handshake(int, int, void *);
...
extern int zenircd_set_tls_groups(SSL_CTX *ctx, const char *groups);
...
extern void do_zen_log_remote_deliver_default_handler(LogLevel loglevel, const char *subsystem, const char *event_id, MultiLine *msg, const char *json_serialized);
...
extern void zendns_delreq_bycptr(Client *cptr);
extern void zendns_gethostbyname_link(const char *name, ConfigItem_link *conf, int ipv4_only);
extern void zendns_delasyncconnects(void);
extern EVENT(zendns_timeout);
extern void zen_free_hostent(struct hostent *he);
extern struct hostent *zen_create_hostent(const char *name, const char *ip);
extern const char *zen_time_sync_error(void);
extern int zen_time_synch(int timeout);
...
extern MODVAR pcre2_match_context *zen_pcre2_match_ctx;
extern void init_match(void);
extern Match *zen_create_match(MatchType type, const char *str, const char **error);
extern void zen_delete_match(Match *m);
extern int zen_match(Match *m, const char *str, const char **error);
extern int zen_match_method_strtoval(const char *str);
extern char *zen_match_method_valtostr(int val);
#ifdef _WIN32
...
extern int zen_bind(int fd, const char *ip, int port, SocketType socket_type);
extern int zen_connect(int fd, const char *ip, int port, SocketType socket_type);
...
extern void zen_del_quotes(char *i);
extern const char *zen_add_quotes(const char *str);
extern int zen_add_quotes_r(const char *i, char *o, size_t len);
...
extern void zen_add_name_values(NameValuePrioList **n, const char *name, ConfigEntry *ce);
...
extern char *zen_strftime(const char *str);
...
extern json_t *json_string_zen(const char *s);
...
extern void zen_delete_masks(ConfigItem_mask *m);
extern void zen_add_masks(ConfigItem_mask **head, ConfigEntry *ce);
extern void zen_add_mask_string(ConfigItem_mask **head, const char *name);
extern ConfigItem_mask *zen_duplicate_masks(ConfigItem_mask *existing);
extern int zen_mask_match(Client *acptr, ConfigItem_mask *m);
extern int zen_mask_match_string(const char *name, ConfigItem_mask *m);
...
/* securitygroup.c end */
/* src/zendb.c start */
extern ZenDB *zendb_open(const char *filename, ZenDBMode mode, char *secret_block);
extern int zendb_close(ZenDB *c);
extern char *zendb_test_db(const char *filename, char *secret_block);
extern int zendb_write_int64(ZenDB *c, uint64_t t);
extern int zendb_write_int32(ZenDB *c, uint32_t t);
extern int zendb_write_int16(ZenDB *c, uint16_t t);
extern int zendb_write_str(ZenDB *c, const char *x);
extern int zendb_write_char(ZenDB *c, char t);
extern int zendb_read_int64(ZenDB *c, uint64_t *t);
extern int zendb_read_int32(ZenDB *c, uint32_t *t);
extern int zendb_read_int16(ZenDB *c, uint16_t *t);
extern int zendb_read_str(ZenDB *c, char **x);
extern int zendb_read_char(ZenDB *c, char *t);
extern const char *zendb_test_secret(const char *name);
extern ZenDBConfig *zendb_copy_config(ZenDBConfig *src);
extern ZenDBConfig *zendb_get_config(ZenDB *db);
extern void zendb_free_config(ZenDBConfig *c);
extern ZenDBError zendb_get_error_code(void);
extern const char *zendb_get_error_string(void);
/* src/zendb.c end */
...
extern const char *log_type_valtostring(LogType v);
  ...
  * there is an issue if zen_log() is used with a parameter like xyz ? log_data_string("zzz") : NULL,
  ...
  */
 #define zen_log(level, sys, id, ...) \
	 do \
	 { \
		 if (!log_throttled((sys), (id))) \
		 { \
			 LogData *lds = log_data_source(__FILE__, __LINE__, __FUNCTION__); \
			 do_zen_log((level), (sys), (id), __VA_ARGS__, lds, NULL); \
			 log_data_free(lds); \
		 } \
	 } while (0)
 #define zen_log_raw(level, sys, id, ...) \
	 do \
	 { \
		 if (!log_throttled((sys), (id))) \
		 { \
			 LogData *lds = log_data_source(__FILE__, __LINE__, __FUNCTION__); \
			 do_zen_log_raw((level), (sys), (id), __VA_ARGS__, lds, NULL); \
			 log_data_free(lds); \
		 } \
	 } while (0)
#else
 #define zen_log(level, sys, id, ...) \
	 ...
	 { \
		 if (!log_throttled((sys), (id))) \
			 do_zen_log((level), (sys), (id), __VA_ARGS__, NULL); \
	 } while (0)
 #define zen_log_raw(level, sys, id, ...) \
	 do \
	 { \
		 if (!log_throttled((sys), (id))) \
			 do_zen_log_raw((level), (sys), (id), __VA_ARGS__, NULL); \
	 } while (0)
#endif
extern void do_zen_log(LogLevel loglevel, const char *subsystem, const char *event_id, Client *client, const char *msg, ...) __attribute__((format(printf, 5, 0)));
extern void do_zen_log_raw(LogLevel loglevel, const char *subsystem, const char *event_id, Client *client, const char *msg, ...);
extern void do_zen_log_internal_from_remote(LogLevel loglevel, const char *subsystem, const char *event_id, MultiLine *msg, json_t *json, const char *json_serialized, Client *from_server);
...
/* url stuff */
extern const char *zen_mkcache(const char *url);
