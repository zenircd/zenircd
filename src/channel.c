/* Zen Internet Relay Chat Daemon, src/channel.c
 * (C) Copyright 1990 Jarkko Oikarinen and
 *                    University of Oulu, Co Center
 * (C) Copyright 1999-present The ZenIRCd team
 *
...
Ban *is_banned_with_nick(Client *client, Channel *channel, int type, const char *nick, const char **msg, const char **errmsg)
{
	 ...
	 * with a non-NULL nick. That doesn't happen much. In ZenIRCd
...
int valid_channelname(const char *cname)
{
	...
	{
		/* The default up to and including ZenIRCd 4 */
		for (p = cname; *p; p++)
		{
			if (*p < 33 || *p == ',' || *p == ':')
				return 0;
 ...
 * {
 *         zen_log(ULOG_INFO, "test", "TEST", "Got %c%c %s",
 *                    pm.what == MODE_ADD ? '+' : '-',
 *                    pm.modechar,
 *                    pm.param ? pm.param : "");
 * }
		...
		} else
		{
			...
			if (eatparam)
			{
				...
				if (pm->parabuf && *pm->parabuf)
				{
					...
					stripcrlf(pm->buf); /* needed for zen_server_compat.c */
					pm->param = pm->buf;
				} else
				{
					pm->modebuf++;
