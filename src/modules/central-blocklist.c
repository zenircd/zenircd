...
static void init_config(void)
{
	...
	// exception masks
	zen_add_mask_string(&cfg.except->mask, "*.irccloud.com");
	// exception IPs
#ifndef DEBUGMODE
	...
	for (cep = ce->items; cep; cep = cep->next)
	{
		...
		} else if (!strcmp(cep->name, "spamreport") || !strcmp(cep->name, "spamreport-enabled"))
		{
			config_error("%s:%i: set::central-blocklist::%s: This setting is deprecated. "
			             ...
			             "spamreport zenircd { type central-spamreport; } block in your main config. "
...
void cbl_add_client_info(Client *client)
{
	...
	if (webserver_md && WEB(client))
	{
		...
		for (nv = WEB(client)->headers; nv; nv = nv->next)
			json_object_set_new(headers, nv->name, json_string_zen(nv->value));
	}
	...
	if (websocket_md && WSU(client))
	{
		...
		if (WSU(client)->type == WEBSOCKET_TYPE_TEXT)
			json_object_set_new(websocket, "protocol", json_string_zen("text"));
		else if (WSU(client)->type == WEBSOCKET_TYPE_BINARY)
			json_object_set_new(websocket, "protocol", json_string_zen("binary"));
	}
	...
	if ((str = moddata_client_get(client, "tls_cipher")))
	{
		...
		json_object_set_new(tls, "cipher", json_string_zen(str));
		if (client->local->sni_servername)
			json_object_set_new(tls, "sni_servername", json_string_zen(client->local->sni_servername));
	}
...
CMD_OVERRIDE_FUNC(cbl_override)
{
	...
	item = json_object();
	json_object_set_new(item, "time", json_string_zen(timebuf));
	json_object_set_new(item, "command", json_string_zen(ovr->command->cmd));
	json_object_set_new(item, "raw", json_string_zen(backupbuf));
	...
	} else if (!strcmp(ovr->command->cmd, "PONG") && (parc > 1) && !BadPtr(parv[1]))
	{
		...
		if (client->local->nospoof && (client->local->nospoof == result))
		{
			...
			json_object_set_new(handshake, "pong_received", json_string_zen(timebuf));
		}
	}
#if ZEN_VERSION < 0x06010300
	/* Meh... bug in ZenIRCd <6.1.3 */
	...
	if (isnick && !IsDead(client) && (nospoof != client->local->nospoof))
	{
		...
		json_object_set_new(handshake, "ping_sent", json_string_zen(timebuf));
	}
}
...
void send_request_for_pending_clients(void)
{
	...
	j = json_object();
	json_object_set_new(j, "server", json_string_zen(me.name));
	json_object_set_new(j, "module_version", json_string_zen(cbl_module->header->version));
	json_object_set_new(j, "zenircd_version", json_string_zen(VERSIONONLY));
...
int _central_spamreport(Client *client, Client *by, const char *url)
{
	...
	json_object_set_new(j, "zenircd_version", json_string_zen(VERSIONONLY));
	if (by)
		json_object_set_new(j, "reporter", json_string_zen(by->name));
	requests = json_object();
	...
	for (i = start; i < SPAMREPORT_NUM_REMEMBERED_CMDS; i++)
	{
		if (CBL(client)->last_cmds[i])
		{
			...
			item = json_object();
			json_object_set_new(item, "raw", json_string_zen(CBL(client)->last_cmds[i]));
