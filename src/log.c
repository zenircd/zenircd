...
void do_zen_log_internal(LogLevel loglevel, const char *subsystem, const char *event_id, Client *client, int expand_msg, const char *msg, va_list vl);
void log_blocks_switchover(void);
void do_zen_log_webhook(LogLevel loglevel, const char *subsystem, const char *event_id, const char *json_serialized);
...
LogData *log_data_source(const char *file, int line, const char *function)
{
	...
	json_object_set_new(j, "file", json_string_zen(file));
	json_object_set_new(j, "line", json_integer(line));
	json_object_set_new(j, "function", json_string_zen(function));
	return d;
}
...
LogData *log_data_socket_error(int fd)
{
	...
	json_object_set_new(j, "error_string", json_string_zen(STRERROR(sockerr)));
	return d;
}
...
LogData *log_data_tls_error(void)
{
	...
	*all_errors = '\0';
	...
	do
	{
		...
		json_object_set_new(obj, "string", json_string_zen(buf));
		json_array_append_new(error_stack, obj);

		if (name == NULL)
		{
			/* Set tls_error.name to the first error that was encountered */
			json_object_set_new(j, "name", json_string_zen(buf));
		}
		strlcat(all_errors, buf, sizeof(all_errors));
		strlcat(all_errors, "\n", sizeof(all_errors));
	} while (e);

	json_object_set_new(j, "all", json_string_zen(all_errors));
...
LogData *log_data_link_block(ConfigItem_link *link)
{
	...
	d->value.object = j = json_object();
	json_object_set_new(j, "name", json_string_zen(link->servername));
	json_object_set_new(j, "file", json_string_zen(link->outgoing.file));
	json_object_set_new(j, "hostname", json_string_zen(link->outgoing.hostname));
	json_object_set_new(j, "ip", json_string_zen(link->connect_ip));
	json_object_set_new(j, "port", json_integer(link->outgoing.port));
	json_object_set_new(j, "class", json_string_zen(link->class->name));
	...
	if (!bind_ip)
		bind_ip = "*";
	json_object_set_new(j, "bind_ip", json_string_zen(bind_ip));
...
void do_zen_log_disk(LogLevel loglevel, const char *subsystem, const char *event_id, MultiLine *msg, json_t *json, const char *json_serialized, Client *from_server)
{
	...
	/* In case of './zenircd configtest': don't write to log file, only to stderr */
	if (loop.config_test)
		return;
	...
	for (l = logs[LOG_DEST_DISK]; l; l = l->next)
	{
		...
		if (l->filefmt)
		{
			char *fname = zen_strftime(l->filefmt);
		...
		if (l->logfd == -1)
		{
			...
			if (l->logfd == -1)
			{
				if (errno == ENOENT)
				{
					...
					zen_create_directory_structure_for_file(l->file, 0777);
					l->logfd = fd_fileopen(l->file, O_CREAT | O_APPEND | O_WRONLY);
				}
				if (l->logfd == -1)
				{
...
void do_zen_log_opers(LogLevel loglevel, const char *subsystem, const char *event_id, MultiLine *msg, const char *json_serialized, Client *from_server)
{
	Client *client;
...
void do_zen_log_channels(LogLevel loglevel, const char *subsystem, const char *event_id, MultiLine *msg, const char *json_serialized, Client *from_server)
{
	Log *l;
	Member *m;
	Client *client;
...
void do_zen_log_remote(LogLevel loglevel, const char *subsystem, const char *event_id, MultiLine *msg, const char *json_serialized)
{
	Log *l;
	int found = 0;
	...
	do_zen_log_remote_deliver(loglevel, subsystem, event_id, msg, json_serialized);
}
...
void webhook_send_async(const char *url, const char *json_data)
{
	...
	add_nvplist(&headers, 0, "User-Agent", "ZenIRCd-Webhook/1.0");
...
void do_zen_log_control(LogLevel loglevel, const char *subsystem, const char *event_id, MultiLine *msg, json_t *j, const char *json_serialized, Client *from_server)
{
	Client *client;
	MultiLine *m;
...
void do_zen_log_free_args(va_list vl)
{
	LogData *d;
...
static int zen_log_recursion_trap = 0;
                ...
                Client *client, const char *msg, ...)
{
	...
	{
		va_start(vl, msg);
		do_zen_log_free_args(vl);
		va_end(vl);
		return;
	}

	zen_log_recursion_trap = 1;
	va_start(vl, msg);
	do_zen_log_internal(loglevel, subsystem, event_id, client, 1, msg, vl);
	va_end(vl);
	zen_log_recursion_trap = 0;
}
                    ...
                    Client *client, const char *msg, ...)
{
	...
	zen_log_recursion_trap = 1;
	va_start(vl, msg);
	do_zen_log_internal(loglevel, subsystem, event_id, client, 0, msg, vl);
	va_end(vl);
	zen_log_recursion_trap = 0;
}
                         ...
                         Client *client, int expand_msg, const char *msg, va_list vl)
{
	...
	/* Set flag so json_string_zen() uses more strict filter */
	log_json_filter = 1;
	...
	json_object_set_new(j, "timestamp", json_string_zen(timestamp_iso8601_now()));
	json_object_set_new(j, "level", json_string_zen(loglevel_string));
	json_object_set_new(j, "subsystem", json_string_zen(subsystem));
	json_object_set_new(j, "event_id", json_string_zen(event_id));
	json_object_set_new(j, "log_source", json_string_zen(*me.name ? me.name : "local"));
	...
	while ((d = va_arg(vl, LogData *)))
	{
		switch (d->type)
		{
			...
			case LOG_FIELD_STRING:
				if (d->value.string)
					json_object_set_new(j_details, d->key, json_string_zen(d->value.string));
				else
			...
			case LOG_FIELD_OPTIONAL_NAME_VALUE:
				...
				json_object_set_new(optional_keys, d->key, json_string_zen(d->label));
				if (d->value.string)
					json_object_set_new(j_details, d->key, json_string_zen(d->value.string));
				else
					json_object_set_new(j_details, d->key, json_null());
	...
	json_object_set_new(j, "msg", json_string_zen(msgbuf));
	...
	do_zen_log_disk(loglevel, subsystem, event_id, mmsg, j, json_serialized, from_server);

	if ((loop.rehashing == 2) || !strcmp(subsystem, "config"))
		do_zen_log_control(loglevel, subsystem, event_id, mmsg, j, json_serialized, from_server);

	do_zen_log_opers(loglevel, subsystem, event_id, mmsg, json_serialized, from_server);

	do_zen_log_channels(loglevel, subsystem, event_id, mmsg, json_serialized, from_server);

	do_zen_log_remote(loglevel, subsystem, event_id, mmsg, json_serialized);

	do_zen_log_webhook(loglevel, subsystem, event_id, json_serialized);

	// NOTE: code duplication further down!

	/* This one should only be in do_zen_log_internal()
	 * and never in do_zen_log_internal_from_remote()
	 */
                                     ...
                                     MultiLine *msg, json_t *json, const char *json_serialized, Client *from_server)
{
	...
	do_zen_log_disk(loglevel, subsystem, event_id, msg, json, json_serialized, from_server);

	/* And to IRC */
	do_zen_log_opers(loglevel, subsystem, event_id, msg, json_serialized, from_server);
	do_zen_log_channels(loglevel, subsystem, event_id, msg, json_serialized, from_server);
