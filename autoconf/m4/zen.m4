...
dnl Macro: zen_CHECK_TYPE_SIZES
dnl originally called unet_CHECK_TYPE_SIZES
dnl
dnl Check the size of several types and define a valid int16_t and int32_t.
dnl
AC_DEFUN([zen_CHECK_TYPE_SIZES],
...
[
  ...
  You have decided to build ZenIRCd with libcURL (remote includes) support.
  However, you have system-installed c-ares support has either been disabled
  (--without-system-cares) or is unavailable.
  Because ZenIRCd will use a bundled copy of c-ares which may be incompatible
  ...
  Or ZenIRCd might even crash.

  Please build ZenIRCd with --with-system-cares when enabling --enable-libcurl
]])
		])
		...
		dnl Choose ZenIRCds internal implementation of url.c
		URL="url_zen.o"
