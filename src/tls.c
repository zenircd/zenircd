/************************************************************************
 *   Zen Internet Relay Chat Daemon, src/tls.c
...
void disable_ssl_protocols(SSL_CTX *ctx, TLSOptions *tlsoptions)
{
	...
	} else if (!(tlsoptions->protocols & TLS_PROTOCOL_TLSV1))
	{
 ...
 */
int zenircd_set_tls_groups(SSL_CTX *ctx, const char *groups)
{
...
SSL_CTX *init_ctx(TLSOptions *tlsoptions, int server)
{
 ...
 #error "Your system has an outdated OpenSSL version. Please upgrade OpenSSL."
	...
	if (tlsoptions->groups == NULL)
	{
		...
		if (!zenircd_set_tls_groups(ctx, ZENIRCD_DEFAULT_TLS_GROUPS_PRIMARY))
		{
			if (!zenircd_set_tls_groups(ctx, ZENIRCD_DEFAULT_TLS_GROUPS_SECONDARY))
			{
...
void zen_tls_client_handshake(int fd, int revents, void *data)
{
	Client *client = data;
	...
	switch (zen_tls_connect(client, fd))
	{
		case -1:
...
static void zen_tls_accept_retry(int fd, int revents, void *data)
{
	Client *client = data;
	zen_tls_accept(client, fd);
}

/** Accept an TLS connection - that is: do the TLS handshake */
int zen_tls_accept(Client *client, int fd)
{
	int ssl_err;
	...
	if ((ssl_err = SSL_accept(client->local->ssl)) <= 0)
	{
		switch (ssl_err = SSL_get_error(client->local->ssl, ssl_err))
		{
			...
			case SSL_ERROR_WANT_READ:
				fd_setselect(fd, FD_SELECT_READ, zen_tls_accept_retry, client);
				fd_setselect(fd, FD_SELECT_WRITE, NULL, client);
				return 1;
			case SSL_ERROR_WANT_WRITE:
				fd_setselect(fd, FD_SELECT_READ, NULL, client);
				fd_setselect(fd, FD_SELECT_WRITE, zen_tls_accept_retry, client);
				return 1;
			default:
				return fatal_tls_error(ssl_err, FUNC_TLS_ACCEPT, ERRNO, client);
		}
...
static void zen_tls_connect_retry(int fd, int revents, void *data)
{
	Client *client = data;
	zen_tls_connect(client, fd);
}

/** Connect to a remote host - that is: connect and do the TLS handshake */
int zen_tls_connect(Client *client, int fd)
{
	int ssl_err;
	...
	if ((ssl_err = SSL_connect(client->local->ssl)) <= 0)
	{
		...
		switch (ssl_err)
		{
			case SSL_ERROR_SYSCALL:
				if (ERRNO == P_EINTR || ERRNO == P_EWOULDBLOCK || ERRNO == P_EAGAIN)
				{
					/* Hmmm. This implementation is different than in zen_tls_accept().
					 * One of them must be wrong -- better check! (TODO)
					 */
					fd_setselect(fd, FD_SELECT_READ | FD_SELECT_WRITE, zen_tls_connect_retry, client);
					return 0;
				}
				return fatal_tls_error(ssl_err, FUNC_TLS_CONNECT, ERRNO, client);
			case SSL_ERROR_WANT_READ:
				fd_setselect(fd, FD_SELECT_READ, zen_tls_connect_retry, client);
				fd_setselect(fd, FD_SELECT_WRITE, NULL, client);
				return 0;
			case SSL_ERROR_WANT_WRITE:
				fd_setselect(fd, FD_SELECT_READ, NULL, client);
				fd_setselect(fd, FD_SELECT_WRITE, zen_tls_connect_retry, client);
				return 0;
			default:
				return fatal_tls_error(ssl_err, FUNC_TLS_CONNECT, ERRNO, client);
		}
...
static int fatal_tls_error(int ssl_error, int where, int my_errno, Client *client)
{
	...
	} else if (IsServer(client) || (client->server && client->server->conf))
	{
...
int client_starttls(Client *client)
{
	...
	if (zen_tls_connect(client, client->local->fd) < 0)
	{
		SSL_set_shutdown(client->local->ssl, SSL_RECEIVED_SHUTDOWN);
		SSL_smart_shutdown(client->local->ssl);
		SSL_free(client->local->ssl);
...
int verify_certificate(SSL *ssl, const char *hostname, char **errstr)
{
	...
	*buf = '\0';
	 ...
	 * An well, TODO: ditch this old TLS support in next major ZenIRCd
...
SSL_CTX *https_new_ctx(void)
{
	...
	SSL_CTX_set_cipher_list(ctx_client, ZENIRCD_DEFAULT_CIPHERS);

	/* And TLS groups */
#if defined(HAS_SSL_CTX_SET1_CURVES_LIST) || defined(HAS_SSL_CTX_SET1_GROUPS_LIST)
	if (!zenircd_set_tls_groups(ctx_client, ZENIRCD_DEFAULT_TLS_GROUPS_PRIMARY))
		if (!zenircd_set_tls_groups(ctx_client, ZENIRCD_DEFAULT_TLS_GROUPS_SECONDARY))
			if (!zenircd_set_tls_groups(ctx_client, ZENIRCD_DEFAULT_TLS_GROUPS_TERTIARY))
				;
#endif
	return ctx_client;
}
