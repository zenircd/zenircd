                               ...
                               int *targets, char *targetbuf, size_t targetbuflen, BanActionValue *action, char *actionbuf)
{
	...
	*match_type = zen_match_method_strtoval(str);
	if (!*match_type)
	{
...
RPC_CALL_FUNC(rpc_spamfilter_del)
{
	...
	tkllayer[8] = "0";
	tkllayer[9] = "-";
	tkllayer[10] = zen_match_method_valtostr(match_type);
	tkllayer[11] = name;
	tkllayer[12] = NULL;
