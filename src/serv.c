/*
 *   Zen Internet Relay Chat Daemon, src/serv.c
...
char *zeninfo[] = {
    ...
    "* i <i@unrealircd.org>",
    "",
    "Past ZenIRCd 4.x coders/contributors:",
    "* Heero, binki, nenolod, ..",
    "",
    "Past ZenIRCd 3.2.x coders/contributors:",
...
CMD_FUNC(cmd_rehash)
{
	...
	if (!BadPtr(parv[1]) && strcasecmp(parv[1], "-all"))
	{
		if (*parv[1] == '-')
		{
			...
			{
				zen_log(ULOG_INFO, "config", "CONFIG_RELOAD_TLS", client, "Reloading all TLS related data. [by: $client.details]");
				reinit_tls();
				return;
			}
...
void read_motd(const char *filename, MOTDFile *themotd)
{
	...
	modtime = zen_getfilemodtime(filename);
