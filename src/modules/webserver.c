/*
 * Webserver
 * (C)Copyright 2016 Bram Matthys and the ZenIRCd team
...
#if CHAR_MIN < 0
 #error "In ZenIRCd char should always be unsigned. Check your compiler"
#endif
...
/** ZenIRCd internals: free WebRequest object. */
void webserver_webrequest_mdata_free(ModData *m)
{
	...
	*lastloc_len = 0;
	...
	if (!foundlf)
	{
		...
		*lastloc_len = buflen - (k - buf);
		/* zen_log(ULOG_DEBUG, "webserver", "WEBSERVER_FRAMING", NULL,
...
int webserver_handle_request_header(Client *client, const char *readbuf, int *length)
{
	...
	safe_free(netbuf);
	return 0; /* don't let ZenIRCd process this */
}
