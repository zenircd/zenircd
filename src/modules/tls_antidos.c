 ...
 *
 * (C) Copyright 2015- Bram Matthys and the ZenIRCd team.
 * 
...
void ssl_info_callback(const SSL *ssl, int where, int ret)
{
	if (where & SSL_CB_HANDSHAKE_START)
	{
		...
		} else
		{
			...
			{
				zen_log(ULOG_INFO, "flood", "TLS_HANDSHAKE_FLOOD", client, "TLS Handshake flood detected from $client.details -- killed");
