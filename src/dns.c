 ...
 * (C) 2005 Bram Matthys (Syzop) and the ZenIRCd Team
 *
...
#if !defined(ZEN_VERSION_TIME)
 #error "YOU MUST RUN ./Config WHENEVER YOU ARE UPGRADING ZENIRCD!!!!"
#endif


/* Prevent crashes due to invalid prototype/ABI.
 * And force the use of at least the version shipped with ZenIRCd
 * (or at least one without known security issues).
 */
#if ARES_VERSION < 0x010600
 #error "You have an old c-ares version on your system and/or ZenIRCd's c-ares failed to compile!"
#endif

#if defined(__GNUC__)
/* Ignore these for src/dns.c & src/url_zen.c
 ...
 * So: we can look into that in some major new ZenIRCd version, nothing urgent,
...
void zendns_cb_iptoname(void *arg, int status, int timeouts, struct hostent *he);
void zendns_cb_nametoip_verify(void *arg, int status, int timeouts, struct hostent *he);
void zendns_cb_nametoip_link(void *arg, int status, int timeouts, struct hostent *he);
void zendns_delasyncconnects(void);
static uint64_t zendns_hash_ip(const char *ip);
static void zendns_addtocache(const char *name, const char *ip);
static const char *zendns_findcache_ip(const char *ip, int *found);
struct hostent *zen_create_hostent(const char *name, const char *ip);
static void zendns_freeandremovereq(DNSReq *r);
void zendns_removecacherecord(DNSCache *c);
...
static unsigned int zendns_num_cache = 0; /**< # of cache entries in memory */

static char siphashkey_dns_ip[SIPHASH_KEY_LENGTH];

static void zendns_io_cb(int fd, int revents, void *data)
{
...
static void zendns_sock_state_cb(void *data, ares_socket_t fd, int read, int write)
{
	int selflags = 0;
	...
	fd_setselect(fd, selflags, zendns_io_cb, data);
}

/* Who thought providing a socket OPEN callback without a socket CLOSE callback was
 * a good idea...?  --nenolod
 */
static int zendns_sock_create_cb(ares_socket_t fd, int type, void *data)
{
...
static Event *zendns_timeout_hdl = NULL;
...
void init_resolver(int firsttime)
{
	...
	options.sock_state_cb = zendns_sock_state_cb;
	...
	ares_set_socket_callback(resolver_channel_client, zendns_sock_create_cb, RESOLVER_CHANNEL_CLIENT);
	...
	ares_set_socket_callback(resolver_channel_https, zendns_sock_create_cb, RESOLVER_CHANNEL_HTTPS);
	...
	ares_set_socket_callback(resolver_channel_dnsbl, zendns_sock_create_cb, RESOLVER_CHANNEL_DNSBL);

	if (!zendns_timeout_hdl)
		zendns_timeout_hdl = EventAdd(NULL, "zendns_timeout", zendns_timeout, NULL, 500, 0);
}

void reinit_resolver(Client *client)
{
	EventDel(zendns_timeout_hdl);
...
void zendns_addreqtolist(DNSReq *r)
{
	if (requests)
	{
		r->next = requests;
 ...
 */
struct hostent *zendns_doclient(Client *client)
{
	DNSReq *r;
	const char *cache_name;
	int found;

	cache_name = zendns_findcache_ip(client->ip, &found);
	if (found && !cache_name)
	{
		/* Negatively cached */
		return zen_create_hostent(NULL, client->ip);
	}
	if (cache_name)
	{
		/* Cached */
		return zen_create_hostent(cache_name, client->ip);
	}
	...
	if (r->ipv6)
	{
		...
		ares_gethostbyaddr(resolver_channel_client, &addr, 16, AF_INET6, zendns_cb_iptoname, r);
	} else
	{
		struct in_addr addr;
		memset(&addr, 0, sizeof(addr));
		inet_pton(AF_INET, client->ip, &addr);
		ares_gethostbyaddr(resolver_channel_client, &addr, 4, AF_INET, zendns_cb_iptoname, r);
	}
 ...
 */
void zendns_gethostbyname_link(const char *name, ConfigItem_link *conf, int ipv4_only)
{
	DNSReq *r;
	...
	/* Execute it */
	ares_gethostbyname(resolver_channel_client, r->name, r->ipv6 ? AF_INET6 : AF_INET, zendns_cb_nametoip_link, r);
}
...
void zendns_cb_iptoname(void *arg, int status, int timeouts, struct hostent *he)
{
	...
	{
		/* Failed */
		zendns_addtocache(NULL, client->ip); /* negcache */
	...
	zendns_addreqtolist(newr);

	ares_gethostbyname(resolver_channel_client, he->h_name, ipv6 ? AF_INET6 : AF_INET, zendns_cb_nametoip_verify, newr);
}
...
void zendns_cb_nametoip_verify(void *arg, int status, int timeouts, struct hostent *he)
{
	...
	zendns_addtocache(r->name, client->ip);
	he2 = zen_create_hostent(r->name, client->ip);
...
void zendns_cb_nametoip_link(void *arg, int status, int timeouts, struct hostent *he)
{
	...
	{
		/* Resolver is shutting down ('REHASH -dns'). We must end this request. */
	...
	if ((status != 0) || !he->h_addr_list || !he->h_addr_list[0])
	{
		if (r->ipv6)
		{
			/* Retry for IPv4... */
			r->ipv6 = 0;
			ares_gethostbyname(resolver_channel_client, r->name, AF_INET, zendns_cb_nametoip_link, r);
	...
	he2 = zen_create_hostent(he->h_name, ip);
...
static void zendns_addtocache(const char *name, const char *ip)
{
	...
	if (zendns_num_cache >= DNS_MAX_ENTRIES)
	{
		for (c = cache_list; c->next; c = c->next)
			;
		zendns_removecacherecord(c);
	}
...
void zen_free_hostent(struct hostent *he)
{
	safe_free(he->h_name);
 ...
 */
void zendns_delreq_bycptr(Client *client)
{
	DNSReq *r;
...
CMD_FUNC(cmd_dns)
{
	...
	{
		sendtxtnumeric(client, "DNS CACHE List (%u items):", zendns_num_cache);
 ...
 */
void zen_gethostbyname_api(const char *name, int family, const char *callback_name, void *arg)
{
