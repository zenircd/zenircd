 ...
 * (C) Copyright 2025-.. Valware and the ZenIRCd team
...
RPC_CALL_FUNC(rpc_security_group_list)
{
	...
	/* Add the magic 'unknown-users' and 'known-users' groups first */
	{
		...
		json_object_set_new(item, "name", json_string_zen("unknown-users"));
...
RPC_CALL_FUNC(rpc_security_group_get)
{
	...
	{
		result = json_object();
		json_object_set_new(result, "name", json_string_zen("unknown-users"));
		json_object_set_new(result, "priority", json_integer(0));
		json_object_set_new(result, "builtin", json_boolean(1));
		json_object_set_new(result, "description", json_string_zen("Users not matching the 'known-users' security group"));
		rpc_response(client, request, result);
		json_decref(result);
		return;
	}
