/************************************************************************
 *   ZenIRCd - Zen Internet Relay Chat Daemon - src/zenircdctl
 *   (c) 2022- Bram Matthys and The ZenIRCd team
 *
...
/** @file
 * @brief ZenIRCd Control
 */
#include "zenircd.h"

#ifdef _WIN32
 #define ZENCMD "zenircdctl"
#else
 #define ZENCMD "./zenircd"
#endif


extern int procio_client(const char *command, int auto_color_logs);

void zenircdctl_usage(const char *program_name)
{
...
void zenircdctl_reloadtls(void)
{
...
void zenircdctl_status(void)
{
	if (procio_client("STATUS", 2) == 0)
	{
		printf("ZenIRCd is up and running.\n");
		exit(0);
	}
	printf("ZenIRCd status report failed.\n");
	exit(1);
}

void zenircdctl_module_status(void)
{
...
void zenircdctl_mkpasswd(int argc, char *argv[])
{
...
void zenircdctl_gencloak(int argc, char *argv[])
{
