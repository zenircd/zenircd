...
char *_zen_expand_string(const char *str, char *buf, size_t buflen, NameValuePrioList *nvp, int buildvarstring_options, Client *client);

MOD_TEST()
{
	MARK_AS_OFFICIAL_MODULE(modinfo);
	EfunctionAdd(modinfo->handle, EFUNC_REGISTER_USER, _register_user);
	EfunctionAddString(modinfo->handle, EFUNC_ZEN_EXPAND_STRING, _zen_expand_string);
	return MOD_SUCCESS;
}
...
/** The NICK command.
 * In ZenIRCd 4 and later this should only happen for:
...
#ifdef EXPERIMENTAL
	sendnotice(client,
	           "*** \2NOTE:\2 This server is running experimental IRC server software (ZenIRCd %s). "
