/************************************************************************
 *   Zen Internet Relay Chat Daemon, src/hash.c
 ...
 * such as dropping the uint8_t stuff (in ZenIRCd char is always unsigned)
 * and getting rid of the length argument.
 *
 * The end result are simple functions for API end-users and we encourage
 * everyone to use these two hash functions everywhere in ZenIRCd.
 */
...
/** Generic hash function in ZenIRCd - raw version.
...
/** Generic hash function in ZenIRCd - case insensitive.
...
/** Generic hash function in ZenIRCd.
 * @param str   The string to hash (NUL-terminated)
 * @param k     The key to use for hashing (SIPHASH_KEY_LENGTH bytes,
 *              which is actually 16, not NUL terminated)
 * @returns Hash result as a 64 bit unsigned integer.
...
void init_hash(void)
{
	...
	if (strcmp(BASE_VERSION, &zenlogo[337]))
		loop.tainted = 1;
}
...
int add_to_client_hash_table(const char *name, Client *client)
{
	 ...
	 * ZenIRCd versions since 3.0. I'm sick of people ripping the IRCd off and 
