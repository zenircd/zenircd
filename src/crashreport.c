...
#include "zenircd.h"
#ifdef _WIN32
extern void StartZenAgain(void);
#endif
#include "version.h"
...
char *find_best_coredump(void)
{
	...
	*best_fname = '\0';
	...
	hFile = FindFirstFile("zenircd.*.core", &hData);
...
char *find_best_asan_log(void)
{
	...
	*best_fname = '\0';
	...
	{
		char *fname = dir->d_name;
		if (strstr(fname, "zenircd_asan.") && !strstr(fname, ".so") &&
		    !strstr(fname, ".conf") && !strstr(fname, ".txt") &&
		    !strstr(fname, ".done"))
		{
			char buf[512];
...
void crash_report_fix_libs(char *coredump, int *thirdpartymods)
{
	...
	snprintf(cmd, sizeof(cmd), "echo info sharedlibrary|gdb %s/zenircd %s 2>&1",
	         BINDIR, coredump);
	...
	while ((fgets(buf, sizeof(buf), fd)))
	{
		 ...
		 * <many spaces>    No        /home/blabla/zenircd/tmp/5114DF16.m_kick.so
		 */
		if (!strstr(buf, " No "))
			continue;
...
int crash_report_backtrace(FILE *reportfd, char *coredump)
{
	...
	snprintf(cmd, sizeof(cmd), "gdb -batch -x %s %s/zenircd %s 2>&1",
	         buf, BINDIR, coredump);
...
void crash_report_header(FILE *reportfd, char *coredump)
{
	...
	fprintf(reportfd, "== ZENIRCD CRASH REPORT ==\n"
	                  "\n"
	                  "SYSTEM INFORMATION:\n");

	fprintf(reportfd, "ZenIRCd version: %s\n", VERSIONONLY);
...
int corefile_vs_binary_mismatch(char *coredump)
{
	...
	snprintf(fname, sizeof(fname), "%s/zenircd", BINDIR);
	...
	return 0; /* GOOD! */
#else
	return 0; /* guess we don't check this on Windows? Or will we check ZenIRCd.exe... hmm.. yeah maybe good idea */
#endif
}
...
char *copy_libc_so(void)
{
	...
	snprintf(buf, sizeof(buf), "ldd %s/zenircd 2>/dev/null", BINDIR);
	fd = popen(buf, "r");
	if (!fd)
		return "";
...
int attach_coredump(FILE *fdo, char *coredump)
{
	...
	printf("Please wait...\n");
	snprintf(fname, sizeof(fname), "tar c %s/zenircd %s %s %s 2>/dev/null|(bzip2 || gzip) 2>/dev/null",
...
char *generate_crash_report(char *coredump, int *thirdpartymods)
{
	...
	*thirdpartymods = 0;
	...
	snprintf(reportfname, sizeof(reportfname), "%s/crash.report.%s.%ld.txt",
	         TMPDIR, zen_getfilename(coredump), (long)time(NULL));
	...
	if ((n < 0) || strncmp(buf, "HTTP/1.1 100", 12))
	{
		...
		if (!strncmp(buf, "HTTP/1.1 403", 12))
		{
			printf("Your crash report was rejected automatically.\n"
			       "This normally means your ZenIRCd version is too old and unsupported.\n"
...
void report_crash(void)
{
	...
	{
		printf("** IMPORTANT **\n"
		       "Your ZenIRCd crashed and you have 3rd party modules loaded (modules created\n"
		       "by someone other than the ZenIRCd team). If you installed new 3rd party\n"
		       ...
		       "it to the ZenIRCd team.\n"
		       "The reason we ask you to do this is because MORE THAN 95%% OF ALL CRASH ISSUES\n"
		       "ARE CAUSED BY 3RD PARTY MODULES and not by an ZenIRCd bug.\n"
		       "\n");
	}
	...
	} else if (report_pref == REPORT_ASK)
	{
		...
		printf("Shall I send a crash report to the ZenIRCd developers?\n");
		if (!thirdpartymods)
			printf("Crash reports help us greatly with fixing bugs that affect you and others\n");
		else
			printf("NOTE: If the crash is caused by a 3rd party module then ZenIRCd devs can't fix that.\n");
		printf("\n");
	...
	{
		printf("\nThe crash report has been sent to the ZenIRCd developers. "
		       "Thanks a lot for helping to make ZenIRCd a better product!\n\n");
	}

#else
	/* Windows */
	if (MessageBox(NULL, "ZenIRCd crashed. May I send a report about this to the ZenIRCd developers? This helps us a lot.",
	               "ZenIRCd crash",
	               MB_YESNO | MB_ICONQUESTION) == IDYES)
	{
		...
		if (crashreport_send(fname))
		{
			MessageBox(NULL, "The crash report has been sent to the ZenIRCd developers. "
			           ...
			           "ZenIRCd crash report sent", MB_ICONINFORMATION | MB_OK);
		}
	}
#endif
...
#ifdef _WIN32
	if (MessageBox(NULL, "Start ZenIRCd again?",
	               "ZenIRCd crash",
	               MB_YESNO | MB_ICONQUESTION) == IDYES)
	{
		StartZenAgain();
