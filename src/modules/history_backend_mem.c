 ...
 * (C) Copyright 2019-2023 Bram Matthys (Syzop) and the ZenIRCd team
...
int hbm_config_test(ConfigFile *cf, ConfigEntry *ce, int type, int *errs)
{
	...
	{
		const char *err;
		if ((err = zendb_test_secret(ce->value)))
		{
...
int hbm_config_posttest(int *errs)
{
	...
	} else if (test.db_secret && test.persist)
	{
		 ...
		 */
		char *errstr = NULL;
		if (test.masterdb && ((errstr = zendb_test_db(test.masterdb, test.db_secret))))
		{
...
void hbm_duplicate_mtags(HistoryLogLine *l, MessageTag *m)
{
	...
	} else
	{
		 ...
		 * as the first zenircd server would have added a msgid to it.
...
static int hbm_read_masterdb(void)
{
	...
	db = zendb_open(test.masterdb, ZENDB_MODE_READ, test.db_secret);
	...
	if (!db)
	{
		...
		{
			config_warn("[history] Unable to open the database file '%s' for reading: %s", test.masterdb, zendb_get_error_string());
			return 0;
		}
	}
	 ...
	 * string:  post hash
	 */
	if (!zendb_read_int32(db, &mdb_version) ||
	    !zendb_read_str(db, &prehash) ||
	    !zendb_read_str(db, &posthash))
	{
		config_error("[history] Read error from database file '%s': %s",
		             test.masterdb, zendb_get_error_string());
		safe_free(prehash);
		safe_free(posthash);
		zendb_close(db);
		return 0;
...
static int hbm_write_masterdb(void)
{
	...
	db = zendb_open(test.masterdb, ZENDB_MODE_WRITE, test.db_secret);
	if (!db)
	{
	...
	mdb_version = 5000;
	if (!zendb_write_int32(db, mdb_version) ||
	    !zendb_write_str(db, hbm_prehash) ||
	    !zendb_write_str(db, hbm_posthash))
	{
		config_error("[history] Unable to write to '%s': %s",
		             test.masterdb, zendb_get_error_string());
		return 0;
...
#define R_SAFE(x) \
	...
	{ \
		if (!(x)) \
		{ \
			config_warn("[history] Read error from database file '%s' (possible corruption): %s", fname, zendb_get_error_string()); \
			R_SAFE_CLEANUP(); \
			return 0; \
		} \
	} while (0)
...
static int hbm_read_db(const char *fname)
{
	...
	db = zendb_open(fname, ZENDB_MODE_READ, cfg.db_secret);
	if (!db)
	{
		config_warn("[history] Unable to open the database file '%s' for reading: %s", fname, zendb_get_error_string());
		return 0;
	}
	...
	if (version > 5001)
	{
		config_warn("[history] Database '%s' has version %lu while we only support %lu. Did you just downgrade ZenIRCd? Sorry this is not suported",
		            fname, (unsigned long)version, (unsigned long)5001);
		zendb_close(db);
		return 0;
	}

	R_SAFE(zendb_read_str(db, &prehash));
	R_SAFE(zendb_read_str(db, &posthash));
	...
	R_SAFE(zendb_read_str(db, &object));
	R_SAFE(zendb_read_int64(db, &max_lines));
	R_SAFE(zendb_read_int64(db, &max_time));
	...
	while (1)
	{
		...
		R_SAFE(zendb_read_int64(db, &line_ts));
		while (1)
		{
			R_SAFE(zendb_read_str(db, &mtag_name));
			R_SAFE(zendb_read_str(db, &mtag_value));
		...
		if (version >= 5001)
		{
			...
			HistoryLogLine *l;
			R_SAFE(zendb_read_int32(db, &concat_flag));
			R_SAFE(zendb_read_int32(db, &continuation_count));
			/* Add the batch head */
			...
			if (continuation_count > 0)
			{
				...
				for (i = 0; i < continuation_count; i++)
				{
					uint32_t cl_concat;
					char *cl_line = NULL;
					R_SAFE(zendb_read_int32(db, &cl_concat));
					R_SAFE(zendb_read_str(db, &cl_line));
					if (l)
					{
						HistoryLogLine *cl;
...
/** Flush all dirty logs to disk on ZenIRCd stop */
static void hbm_flush(void)
{
	int hashnum;
	HistoryLogObject *h;
 ...
 * when ZenIRCd is terminating or someone comments the module out
...
static int hbm_write_db(HistoryLogObject *h)
{
	...
	W_SAFE(zendb_write_int32(db, HISTORYDB_MAGIC_FILE_START));
	W_SAFE(zendb_write_int32(db, 5001)); /* VERSION */
	W_SAFE(zendb_write_str(db, hbm_prehash));
	W_SAFE(zendb_write_str(db, hbm_posthash));
	W_SAFE(zendb_write_str(db, h->name));

	W_SAFE(zendb_write_int64(db, h->max_lines));
	W_SAFE(zendb_write_int64(db, h->max_time));

	for (l = h->head; l; l = l->next)
	{
		W_SAFE(zendb_write_int32(db, HISTORYDB_MAGIC_ENTRY_START));
		W_SAFE(zendb_write_int64(db, l->t));
		for (m = l->mtags; m; m = m->next)
		{
			W_SAFE(zendb_write_str(db, m->name));
			W_SAFE(zendb_write_str(db, m->value)); /* can be NULL */
		}
		W_SAFE(zendb_write_str(db, NULL));
		W_SAFE(zendb_write_str(db, NULL));
		W_SAFE(zendb_write_str(db, l->line));
		/* v5001: multiline batch data */
		W_SAFE(zendb_write_int32(db, l->concat));
		{
			HistoryLogLine *b;
			int32_t continuation_count = 0;
			for (b = l->next_in_batch; b; b = b->next_in_batch)
				continuation_count++;
			W_SAFE(zendb_write_int32(db, continuation_count));
			for (b = l->next_in_batch; b; b = b->next_in_batch)
			{
				W_SAFE(zendb_write_int32(db, b->concat));
				W_SAFE(zendb_write_str(db, b->line));
			}
		}
		W_SAFE(zendb_write_int32(db, HISTORYDB_MAGIC_ENTRY_END));
	}
	W_SAFE(zendb_write_int32(db, HISTORYDB_MAGIC_FILE_END));
