...
CMD_FUNC(cmd_mode)
{
	...
	if (parv[2] && (*parv[2] == '&'))
	{
		/* We don't do any bounce-mode handling anymore since ZenIRCd 6 */
		return;
	}
...
void _do_mode(Channel *channel, Client *client, MessageTag *recv_mtags, int parc, const char *parv[], time_t sendts, int samode)
{
	...
	if (IsServer(client))
	{
		if (sendts > 0)
		{
			...
			} else if (sendts < channel->creationtime)
			{
	...
	for (i = 0; i < m->numlines; i++)
	{
		...
		} else
		{
			 ...
			 * be a remote server like ZenIRCd 5.
