/*
 *   Zen Internet Relay Chat Daemon, src/misc.c
 *   Copyright (C) 1990 Jarkko Oikarinen and
 *                      University of Oulu, Computing Center
 *   Copyright (C) 1999-present ZenIRCd team
 *
 ...
 * again after variable expansion (by zen_expand_string usually).
 */
 ...
 */
char *zen_decodespace(const char *s)
{
	const char *i;
 ...
 * This is used for the spamfilter reason.
 */
char *zen_encodespace(const char *s)
{
	const char *i;
	static char buf[512], *o;
...
void zen_add_names(NameList **n, ConfigEntry *ce)
{
	if (ce->items)
	{
		ConfigEntry *cep;
...
void zen_add_name_values(NameValuePrioList **n, const char *name, ConfigEntry *ce)
{
	if (ce->items)
	{
		ConfigEntry *cep;
...
void do_zen_log_remote_deliver_default_handler(LogLevel loglevel, const char *subsystem, const char *event_id, MultiLine *msg, const char *json_serialized)
{
}
...
int terminal_supports_color(void)
{
	 ...
	 * or more may be redirected (bin/zenircd >log 2>&1),
